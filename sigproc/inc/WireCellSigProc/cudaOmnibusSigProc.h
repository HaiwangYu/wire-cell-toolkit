#ifndef WIRECELLSIGPROC_CUOMNIBUSSIGPROC
#define WIRECELLSIGPROC_CUOMNIBUSSIGPROC
...
//#include "../../../cuda/inc/WireCellCuda/simplecudatest.h"
...
#ifdef HAVE_CUDA_H
#define MAX_ROW_M_R_DATA_DEVICE 1000
#define MAX_COL_M_R_DATA_DEVICE 6000
#endif
...
namespace WireCell {
  namespace SigProc {
    ...
    class cudaOmnibusSigProc : public WireCell::IFrameFilter, public WireCell::IConfigurable {
    public:
      cudaOmnibusSigProc(const std::string& anode_tn = "AnodePlane",
...
#ifdef HAVE_CUDA_H
      void init_cudaOmnibusSigProc_CUDA();
      void clean_cudaOmnibusSigProc_CUDA();

      void upload_c_data(std::complex<float>* c, int row, int col);
#endif
...
#ifdef HAVE_CUDA_H
      void restore_baseline_cuda(WireCell::Array::array_xxf& arr);
      void restore_baseline_CUDA(float* data, int row, int col);

      void decon_2D_tightROI_CUDA(int row, int col, float* filter, float* r_data, int start_row, int start_col, int row_len, int col_len);

#endif
      // This little struct is used to map between WCT channel idents
      // and internal cudaOmnibusSigProc wire/channel numbers.  See
...
#ifdef HAVE_CUDA_H
      float* m_r_data_D;
      cufftComplex* m_c_data_D;
      float* m_c_data_temp_D;
      float* roi_filter_D;
      float* signal_D;
      float* temp_signal_D;
  
      thrust::device_ptr<float> signal_dev_ptr;
      thrust::device_ptr<float> temp_signal_dev_ptr;

      thrust::device_ptr<int> signal_flag_dev_ptr;
      thrust::device_ptr<int> temp_signal_flag_dev_ptr;

      thrust::device_ptr<float> temp_row_dev_ptr;

      cudaStream_t m_Streams[STREAM_NUM];
