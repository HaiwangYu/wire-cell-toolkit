...
SimpleChannelNoiseDB::get_filter(int channel, const filter_vector_t &fv) const
{
    const int ind = chind(channel);
    // std::cerr << "ch=" << channel << " ind=" << ind << " " << fv.size() <<
    // std::endl;
...
const IChannelNoiseDatabase::filter_t &
SimpleChannelNoiseDB::rcrc(int channel) const
{
    return get_filter(channel, m_rcrc);
}

const IChannelNoiseDatabase::filter_t &
SimpleChannelNoiseDB::config(int channel) const
{
    return get_filter(channel, m_config);
}

const IChannelNoiseDatabase::filter_t &
SimpleChannelNoiseDB::noise(int channel) const
{
    return get_filter(channel, m_masks);
}

const IChannelNoiseDatabase::filter_t &
SimpleChannelNoiseDB::response(int channel) const
{
...
void SimpleChannelNoiseDB::set_sampling(double tick, int nsamples)
{
    ...
    {
        // std::cerr << "Sampling unchanged: " << nsamples << " @ " <<
        // m_tick/units::ms << " ms" << std::endl;
        return;
    }
    m_nsamples = nsamples;
    m_tick = tick;
...
void SimpleChannelNoiseDB::set_nominal_baseline(
    const std::vector<int> &channels, double baseline)
{
    // std::cerr << "SimpleChannelNoiseDB: set baseline to " << channels.size() <<
    // " chans: " << baseline << std::endl;
                                             ...
                                             double rcrc)
{
    ...
    // auto signal = rcres.generate(WireCell::Waveform::Domain(0,
    // m_nsamples*m_tick), m_nsamples);
    ...
    // std::cout << rcrc << " " << m_tick << " " << m_nsamples << " " <<
    // signal.front() << " " << signal.at(1) << " " << signal.at(2) << std::endl;
    ...
    //   std::complex<float>
    //   temp(real_part*real_part-imag_part*imag_part,2*real_part*imag_part);
...
{
    // std::cerr << "SimpleChannelNoiseDB: set respnose on " << channels.size() <<
    // " chans with: " << spectrum.size() << " samples\n";
                                              ...
                                              double to_shaping)
{
    ...
    // std::cerr << "SimpleChannelNoiseDB: set gain/shaping on " <<
    // channels.size() << " chans to: "
    //           << "g=" << from_gain << "->" << to_gain << ", "
...
void SimpleChannelNoiseDB::set_response_offset(const std::vector<int> &channels,
                                               double offset)
{
    // std::cerr << "SimpleChannelNoiseDB: set response offset " <<
    // channels.size() << " chans to: " << offset << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
...
void SimpleChannelNoiseDB::set_min_rms_cut(const std::vector<int> &channels,
                                           double min_rms)
{
    // std::cerr << "SimpleChannelNoiseDB: set min rms cut on
    // "<<channels.size()<<":[" << channels.front() << "," << channels.back() << "]
    // to: " << min_rms << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
...
{
    // std::cerr << "SimpleChannelNoiseDB: set min rms cut on " << ch << " to: "
    // << min_rms << std::endl;
    int ind = chind(ch);
    set_one(ind, min_rms, m_min_rms, m_default_min_rms);
}
...
void SimpleChannelNoiseDB::set_max_rms_cut(const std::vector<int> &channels,
                                           double max_rms)
{
    // std::cerr << "SimpleChannelNoiseDB: set max rms cut on
    // "<<channels.size()<<":[" << channels.front() << "," << channels.back() << "]
    // to: " << max_rms << std::endl;
    for (auto ch : channels)
    {
...
{
    // std::cerr << "SimpleChannelNoiseDB: set max rms cut on " << ch << " to: "
    // << max_rms << std::endl;
    int ind = chind(ch);
    set_one(ind, max_rms, m_max_rms, m_default_max_rms);
}

void SimpleChannelNoiseDB::set_pad_window_front(
    const std::vector<int> &channels, int pad_f)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window front on " <<
    // channels.size() << " channels: " << pad_f << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
...
void SimpleChannelNoiseDB::set_pad_window_back(const std::vector<int> &channels,
                                               int pad_b)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
...
void SimpleChannelNoiseDB::set_coherent_nf_decon_limit(
    const std::vector<int> &channels, float decon_limit)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
...
void SimpleChannelNoiseDB::set_coherent_nf_decon_lf_cutoff(
    const std::vector<int> &channels, float decon_lf_cutoff)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
...
void SimpleChannelNoiseDB::set_coherent_nf_decon_limit1(
    const std::vector<int> &channels, float decon_limit1)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
...
void SimpleChannelNoiseDB::set_coherent_nf_adc_limit(
    const std::vector<int> &channels, float adc_limit)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
...
void SimpleChannelNoiseDB::set_coherent_nf_protection_factor(
    const std::vector<int> &channels, float protection_factor)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
        set_one(ind, protection_factor, m_protection_factor,
                m_default_protection_factor);
    }
}

void SimpleChannelNoiseDB::set_coherent_nf_min_adc_limit(
    const std::vector<int> &channels, float min_adc_limit)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
...
void SimpleChannelNoiseDB::set_coherent_nf_roi_min_max_ratio(
    const std::vector<int> &channels, float roi_min_max_ratio)
{
    // std::cerr << "SimpleChannelNoiseDB: set pad window back on " <<
    // channels.size() << " channels: " << pad_b << std::endl;
    for (auto ch : channels)
    {
        int ind = chind(ch);
        set_one(ind, roi_min_max_ratio, m_roi_min_max_ratio,
                m_default_roi_min_max_ratio);
    }
}

void SimpleChannelNoiseDB::set_filter(const std::vector<int> &channels,
                                      const multimask_t &masks)
{
    Waveform::compseq_t spectrum;
    ...
    for (auto m : masks)
    {
        ...
        // std::cerr << "set freqmasks: " << get<1>(m) << " " << get<2>(m) << " " <<
        // get<0>(m) << " " << spectrum.at(0) << " " << spectrum.at(169) << " " <<
        // spectrum.at(170)<< std::endl;
    }
    auto filt = std::make_shared<filter_t>(spectrum);
...
void SimpleChannelNoiseDB::set_channel_groups(
    const std::vector<channel_group_t> &channel_groups)
{
    // std::cerr << "SimpleChannelNoiseDB: channel groups: " <<
    // channel_groups.size() << " X " << channel_groups[0].size() << std::endl;
    m_channel_groups = channel_groups;
}

void SimpleChannelNoiseDB::set_bad_channels(const channel_group_t &bc)
{
    // std::cerr << "SimpleChannelNoiseDB:: bad channels: " << bc.size() <<
    // std::endl;
    m_bad_channels = bc;
}
