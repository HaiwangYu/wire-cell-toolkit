...
PMTNoiseROI::PMTNoiseROI(int start_bin, int end_bin, int peak, int channel,
                         float peak_height)
  : start_bin(start_bin)
  , end_bin(end_bin)
{
...
{
    if (find(induction_uwires.begin(), induction_uwires.end(), wire_no) ==
        induction_uwires.end())
    {
...
{
    if (find(induction_vwires.begin(), induction_vwires.end(), wire_no) ==
        induction_vwires.end())
    {
...
bool PMTNoiseROI::merge_ROI(PMTNoiseROI &ROI)
{
    ...
    if ((peaks.at(0) >= ROI.get_start_bin() &&
         peaks.at(0) <= ROI.get_end_bin()) ||
        (start_bin <= ROI.get_peaks().at(0) &&
         end_bin >= ROI.get_peaks().at(0)))
    {
        ...
        {
            if (find(peaks.begin(), peaks.end(), ROI.get_peaks().at(i)) ==
                peaks.end())
        ...
        {
            if (find(collection_wwires.begin(), collection_wwires.end(),
                     ROI.get_wwires().at(i)) == collection_wwires.end())
            {
                collection_wwires.push_back(ROI.get_wwires().at(i));
            }

            // calculate wire height
            if (wwires_peak_heights.find(ROI.get_wwires().at(i)) ==
                wwires_peak_heights.end())
            {
                wwires_peak_heights[ROI.get_wwires().at(i)] =
                    ROI.get_wwires_peak_heights()[ROI.get_wwires().at(i)];
            }
            else
            {
                if (ROI.get_wwires_peak_heights()[ROI.get_wwires().at(i)] >
                    wwires_peak_heights[ROI.get_wwires().at(i)])
                    wwires_peak_heights[ROI.get_wwires().at(i)] =
                        ROI.get_wwires_peak_heights()[ROI.get_wwires().at(i)];
            }
