...
Derivations::CalcRMS(const WireCell::Waveform::realseq_t &signal)
{
...
WireCell::Waveform::realseq_t Derivations::CalcMedian(
    const WireCell::IChannelFilter::channel_signals_t &chansig)
{
    float max_rms = 0;
    ...
    std::vector<float> content(nchannel * nbins,
                               0.0);  // 2D array [channel*nbins + bin]
    ...
    for (int ibin = 0; ibin != nbins; ibin++)
    {
        ...
        for (int ich = 0; ich != nchannel; ich++)
        {
            ...
            if (fabs(cont) < 5 * max_rms && fabs(cont) > 0.001)
            {
                temp.push_back(cont);
            }
        }
