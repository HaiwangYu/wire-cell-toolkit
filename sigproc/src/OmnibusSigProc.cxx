...
OmnibusSigProc::OmnibusSigProc(
    const std::string &anode_tn, const std::string &per_chan_resp_tn,
    const std::string &field_response, double fine_time_offset,
    double coarse_time_offset, double gain, double shaping_time,
    double inter_gain, double ADC_mV, float th_factor_ind, float th_factor_col,
    int pad, float asy, int rebin, double l_factor, double l_max_th,
    double l_factor1, int l_short_length, int l_jump_one_bin,
    double r_th_factor, double r_fake_signal_low_th,
    double r_fake_signal_high_th, double r_fake_signal_low_th_ind_factor,
    double r_fake_signal_high_th_ind_factor, int r_pad, int r_break_roi_loop,
    double r_th_peak, double r_sep_peak, double r_low_peak_sep_threshold_pre,
    int r_max_npeaks, double r_sigma, double r_th_percent,
    std::vector<int> process_planes, int charge_ch_offset,
    const std::string &wiener_tag, const std::string &wiener_threshold_tag,
    const std::string &decon_charge_tag, const std::string &gauss_tag,
    bool use_roi_debug_mode, const std::string &tight_lf_tag,
    const std::string &loose_lf_tag, const std::string &cleanup_roi_tag,
    const std::string &break_roi_loop1_tag,
    const std::string &break_roi_loop2_tag, const std::string &shrink_roi_tag,
    const std::string &extend_roi_tag, const std::string &mp3_roi_tag,
  ...
  , log(Log::logger("sigproc"))
{
    ...
    // std::cout << m_anode->channels().size() << " " << nwire_u << " " << nwire_v
    // << " " << nwire_w << std::endl;
}
...
{
    std::stringstream ss;
    ss << "OspChan<c:" << channel << ",w:" << wire << ",p:" << plane
       << ",i:" << ident << ">";
    return ss.str();
}
...
void OmnibusSigProc::configure(const WireCell::Configuration &config)
{
    ...
    {
        log->warn("OmnibusSigProc has not setting \"nticks\", ignoring value {}",
                  config["nticks"].asInt());
    }
    // m_period = get(config,"period",m_period);
    if (!config["period"].isNull())
    {
        log->warn("OmnibusSigProc has not setting \"period\", ignoring value {}",
                  config["period"].asDouble());
    }
    ...
    m_r_fake_signal_low_th =
        get(config, "r_fake_signal_low_th", m_r_fake_signal_low_th);
    m_r_fake_signal_high_th =
        get(config, "r_fake_signal_high_th", m_r_fake_signal_high_th);
    m_r_fake_signal_low_th_ind_factor =
        get(config, "r_fake_signal_low_th_ind_factor",
            m_r_fake_signal_low_th_ind_factor);
    m_r_fake_signal_high_th_ind_factor =
        get(config, "r_fake_signal_high_th_ind_factor",
            m_r_fake_signal_high_th_ind_factor);
    ...
    m_r_low_peak_sep_threshold_pre = get(config, "r_low_peak_sep_threshold_pre",
    ...
    m_wiener_threshold_tag =
        get(config, "wiener_threshold_tag", m_wiener_threshold_tag);
    ...
    m_use_roi_debug_mode =
        get(config, "use_roi_debug_mode", m_use_roi_debug_mode);
    ...
    m_break_roi_loop1_tag =
        get(config, "break_roi_loop1_tag", m_break_roi_loop1_tag);
    m_break_roi_loop2_tag =
        get(config, "break_roi_loop2_tag", m_break_roi_loop2_tag);
    m_shrink_roi_tag = get(config, "shrink_roi_tag", m_shrink_roi_tag);
    m_extend_roi_tag = get(config, "extend_roi_tag", m_extend_roi_tag);

    m_use_multi_plane_protection = get<bool>(config, "use_multi_plane_protection",
                                             m_use_multi_plane_protection);
    ...
    std::stringstream ss;
    ss << "OmnibusSigproc: internal channel map for tags: gauss:\"" << m_gauss_tag
       << "\", wiener:\"" << m_wiener_tag << "\", frame:\"" << m_frame_tag
       << "\"\n";

    for (auto face : m_anode->faces())
    {
        if (!face)
        {              // A null face means one sided AnodePlane.
        ...
        for (auto plane : face->planes())
        {
            ...
            ss << "\tpind" << plane_index << " "
               ...
               << "fid" << face->ident() << " "
               << "pid" << plane->ident() << " "
               << "cid" << ichans.front()->ident() << " -> cid"
               << ichans.back()->ident() << ", "
               << "cind" << ichans.front()->index() << " -> cind"
               << ichans.back()->index() << ", "
               << "(n=" << pchans.size() << ")\n";
        }
    }
    ...
    for (int iplane = 0; iplane < 3; ++iplane)
    {
        ...
        for (auto ichan : plane_channels[iplane])
        {
            ...
            // << osp_channel_number << " " << osp_wire_number << std::endl;
            m_roi_ch_ch_ident[osp_channel_number] = wct_chan_ident;
            m_channel_map[wct_chan_ident] =
                och;  // we could save some space by storing
            m_channel_range[iplane].push_back(
                och);  // wct ident here instead of a whole och.
            ++osp_wire_number;
            ++osp_channel_number;
        }
    }
...
WireCell::Configuration OmnibusSigProc::default_configuration() const
{
    ...
    cfg["use_multi_plane_protection"] =
        m_use_multi_plane_protection;  // default false
...
void OmnibusSigProc::load_data(const input_pointer &in, int plane)
{
    ...
    for (auto trace : *traces.get())
    {
        ...
        // fixme: this code uses tbin() but other places in this file will barf if
        // tbin!=0.
        int tbin = trace->tbin();
        ...
        {
            m_r_data[plane](och.wire + m_pad_nwires[plane], tbin + qind) =
                charges[qind];
        }
...
void OmnibusSigProc::save_data(ITrace::vector &itraces,
                               IFrame::trace_list_t &indices, int plane,
                               ...
                               IFrame::trace_summary_t &threshold)
{
    ...
    {  // ordered by osp channel
        ...
        for (int itick = 0; itick != m_nticks; itick++)
        {
            ...
            if (m_use_roi_debug_mode)
                ...
                charge.at(itick) =
                    q > 0.0 ? q : 0.0;  // default mode: only save positive
        }
        {
            auto &bad = m_cmm["bad"];
    ...
    else
    {
        ...
        log->debug(
            "OmnibusSigProc::save_data plane index: Qtot={} added {} traces "
            "to total {} indices:[{},{}]",
            plane, qtot, nadded, indices.size(), indices.front(),
            indices.back());
    }
}

// save ROI into the out frame
void OmnibusSigProc::save_roi(
    ...
    std::vector<std::list<SignalROI *>> &roi_channel_list)
{
    ...
    {  // ordered by osp channel
        ...
        // std::cout << "[wgu] wire: " << och.wire << " roi_channel_list.size(): "
        // << roi_channel_list.size() << std::endl;

        std::fill(charge.begin(), charge.end(), 0);

        // for (auto it = roi_channel_list.at(och.wire).begin(); it!=
        // roi_channel_list.at(och.wire).end(); it++){
        //   SignalROI *roi =  *it;
        //   int start = roi->get_start_bin();
        //   int end = roi->get_end_bin();
        //   std::cout << "[wgu] wire: " << och.wire << " ROI: " << start << " " <<
        //   end << " channel: " << roi->get_chid() << " plane: " <<
        //   roi->get_plane() << std::endl;
        // }
        ...
        for (auto signal_roi : roi_channel_list.at(och.wire))
        {
            ...
            // if (och.wire==732)
            //   std::cout << "[wgu] OmnibusSigProc::save_roi() wire: " << och.wire <<
            //   " channel: " << och.channel << " ROI: " << start << " " << end << "
            //   channel: " << signal_roi->get_chid() << " plane: " <<
            //   signal_roi->get_plane() << " max height: " <<
            //   signal_roi->get_max_height() <<  std::endl;
            if (start < 0 or end < 0)
                continue;
            for (int i = start; i <= end; i++)
            {
                if (i - prev_roi_end < 2)
                    continue;        // skip one bin for visibility of two adjacent ROIs
        ...
        if (m_sparse)
        {
            ...
            while (i1 != end)
            {
                ...
                indices.push_back(trace_index);
                itraces.push_back(ITrace::pointer(trace));
                // if (och.wire==67) std::cout << "[wgu] och channel: " << och.channel
                // << " wire: " << och.wire << " plane: " << och.plane << " ident: " <<
                // och.ident << " tbin: " << tbin << " len: " << i2-i1 << std::endl;
...
void OmnibusSigProc::save_ext_roi(
    ITrace::vector &itraces, IFrame::trace_list_t &indices, int plane,
    std::vector<std::list<SignalROI *>> &roi_channel_list)
{
    // reuse this temporary vector to hold charge for a channel.
...
void OmnibusSigProc::init_overall_response(IFrame::pointer frame)
{
    m_period = frame->tick();
    {
        ...
        log->debug("OmnibusSigProc: nticks={} tbinmin={} tbinmax={}", m_nticks,
                   tbinmin, tbinmax);
        ...
        else
        {
            ...
            log->debug("OmnibusSigProc: enlarge window from {} to {}", m_nticks,
                       m_fft_nticks);
        }
        //
    ...
    for (int i = 0; i != 3; i++)
    {
        ...
        {
            m_fft_nwires[i] =
                fft_best_length(m_nwires[i] + fravg.planes[0].paths.size() - 1, 1);
            log->debug(
                "OmnibusSigProc: enlarge wire number in plane {} from {} to {}", i,
                m_nwires[i], m_fft_nwires[i]);
        }
        m_pad_nwires[i] = (m_fft_nwires[i] - m_nwires[i]) / 2;
    }

    // since we only do FFT along time, no need to change dimension for wire ...
    const size_t fine_nticks =
        fft_best_length(fravg.planes[0].paths[0].current.size());
    ...
    {
        auto arr =
            Response::as_array(fravg.planes[iplane], fine_nwires, fine_nticks);
        ...
        for (int irow = 0; irow < fine_nwires; ++irow)
        {
            ...
            for (int i = 0; i != m_fft_nticks; i++)
            {
                ...
                if (fcount < fine_nticks)
                {
                    wfs.at(i) = ((ctime - ftbins.at(fcount - 1)) / fravg.period *
                                     arr(irow, fcount - 1) +
                                 (ftbins.at(fcount) - ctime) / fravg.period *
                                     arr(irow, fcount));  // / (-1);
                }
                else
                {
...
void OmnibusSigProc::decon_2D_init(int plane)
{
    ...
    if (!m_per_chan_resp.empty())
    {
        ...
        // WireCell::Binning tbins(m_nticks, 0-m_period/2.,
        // m_nticks*m_period-m_period/2.); Response::ColdElec ce(m_gain,
        // m_shaping_time);

        // temporary hack ...
        // float scaling = 1./(1e-9*0.5/1.13312);
        // WireCell::Binning tbins(m_nticks, (-5-0.5)*m_period,
        // (m_nticks-5-0.5)*m_period-m_period); Response::ColdElec ce(m_gain*scaling,
        // m_shaping_time);
        //// this is moved into wirecell.sigproc.main production of
        //// microboone-channel-responses-v1.json.bz2
        WireCell::Binning tbins(m_fft_nticks, cr_bins.min(),
                                cr_bins.min() + m_fft_nticks * m_period);
        Response::ColdElec ce(m_gain, m_shaping_time);
    ...
    // response part ...
    Array::array_xxf r_resp =
        Array::array_xxf::Zero(m_r_data[plane].rows(), m_fft_nticks);
    ...
    const std::vector<std::string> filter_names{"Wire_ind", "Wire_ind",
                                                "Wire_col"};
    ...
    const int ncols = m_r_data[plane].cols();
    {
        ...
        arr1 = m_r_data[plane].block(nrows - m_wire_shift[plane], 0,
                                     m_wire_shift[plane], ncols);
        ...
        m_r_data[plane].block(m_wire_shift[plane], 0, nrows - m_wire_shift[plane],
                              ncols) = arr2;
    }
...
void OmnibusSigProc::decon_2D_ROI_refine(int plane)
{
    ...
    Array::array_xxc c_data_afterfilter(m_c_data[plane].rows(),
                                        m_c_data[plane].cols());
    for (int irow = 0; irow < m_c_data[plane].rows(); ++irow)
    {
        for (int icol = 0; icol < m_c_data[plane].cols(); ++icol)
        {
            c_data_afterfilter(irow, icol) =
                m_c_data[plane](irow, icol) * roi_hf_filter_wf.at(icol);
        }
    }
...
void OmnibusSigProc::decon_2D_looseROI(int plane)
{
    ...
    for (auto och : m_channel_range[plane])
    {
        ...
        {
            c_data_afterfilter(irow, icol) =
                m_c_data[plane](irow, icol) * roi_hf_filter_wf2.at(icol);
        }
    }
...
// return true if any channels w/in +/- nnn, inclusive, of the channel has the
// mask.
bool OmnibusSigProc::masked_neighbors(const std::string &cmname, OspChan &ochan,
                                      int nnn)
{
...
bool OmnibusSigProc::operator()(const input_pointer &in, output_pointer &out)
{
    ...
    if (ntraces)
    {
        log->debug("OmnibusSigProc: receive frame {} with {} traces", in->ident(),
                   ntraces);
    }
    else
    {
    ...
    IFrame::trace_list_t tight_lf_traces, loose_lf_traces, cleanup_roi_traces,
        break_roi_loop1_traces, break_roi_loop2_traces, shrink_roi_traces,
        extend_roi_traces;
    ...
    ROI_formation roi_form(m_cmm, m_nwires[0], m_nwires[1], m_nwires[2], m_nticks,
                           m_th_factor_ind, m_th_factor_col, m_pad, m_asy,
                           m_rebin, m_l_factor, m_l_max_th, m_l_factor1,
                           m_l_short_length, m_l_jump_one_bin);
    ROI_refinement roi_refine(
        m_cmm, m_nwires[0], m_nwires[1], m_nwires[2], m_r_th_factor,
        m_r_fake_signal_low_th, m_r_fake_signal_high_th,
        m_r_fake_signal_low_th_ind_factor, m_r_fake_signal_high_th_ind_factor,
        m_r_pad, m_r_break_roi_loop, m_r_th_peak, m_r_sep_peak,
        m_r_low_peak_sep_threshold_pre, m_r_max_npeaks, m_r_sigma, m_r_th_percent,
        m_isWrapped);  //

    const std::vector<float> *perplane_thresholds[3] = {
        &roi_form.get_uplane_rms(), &roi_form.get_vplane_rms(),
        &roi_form.get_wplane_rms()};

    for (int iplane = 0; iplane != 3; ++iplane)
    {
        auto it =
            std::find(m_process_planes.begin(), m_process_planes.end(), iplane);
        if (it == m_process_planes.end())
            continue;
        ...
        if (m_use_roi_debug_mode)
            save_data(*itraces, tight_lf_traces, iplane, perwire_rmses, dummy);
    ...
    for (int iplane = 0; iplane != 3; ++iplane)
    {
        ...
        if (m_use_multi_plane_protection)
        {
            roi_refine.MultiPlaneProtection(iplane, m_anode, m_roi_ch_ch_ident,
                                            roi_form, 1000, m_anode->ident() % 2);
            save_mproi(*itraces, mp3_roi_traces, iplane, roi_refine.get_mp3_rois());
            roi_refine.MultiPlaneROI(iplane, m_anode, m_roi_ch_ch_ident, roi_form,
                                     1000, m_anode->ident() % 2);
    ...
    for (int iplane = 0; iplane != 3; ++iplane)
    {
        ...
        save_data(*itraces, perframe_traces[iplane], iplane, perwire_rmses,
                  thresholds);
        wiener_traces.insert(wiener_traces.end(), perframe_traces[iplane].begin(),
                             perframe_traces[iplane].end());

        decon_2D_charge(iplane);
        std::vector<double> dummy_thresholds;
        if (m_use_roi_debug_mode)
        {
            save_data(*itraces, decon_charge_traces, iplane, perwire_rmses,
                      thresholds);
        }
        roi_refine.apply_roi(iplane, m_r_data[iplane]);
        // roi_form.apply_roi(iplane, m_r_data[plane],1);
    ...
    log->debug(
        ...
        itraces->size(), wiener_traces.size(), m_wiener_tag,
        decon_charge_traces.size(), m_decon_charge_tag, gauss_traces.size(),
