...
OmnibusSigProc::OmnibusSigProc(
    ...
    double fine_time_offset, double coarse_time_offset, double gain, double shaping_time, double inter_gain,
    double ADC_mV, float th_factor_ind, float th_factor_col, int pad, float asy, int rebin, double l_factor,
    double l_max_th, double l_factor1, int l_short_length, int l_jump_one_bin, double r_th_factor,
    double r_fake_signal_low_th, double r_fake_signal_high_th, double r_fake_signal_low_th_ind_factor,
    double r_fake_signal_high_th_ind_factor, int r_pad, int r_break_roi_loop, double r_th_peak, double r_sep_peak,
    double r_low_peak_sep_threshold_pre, int r_max_npeaks, double r_sigma, double r_th_percent,
    std::vector<int> process_planes, int charge_ch_offset, const std::string &wiener_tag,
...
bool OmnibusSigProc::operator()(const input_pointer &in, output_pointer &out)
{
    ...
    ROI_formation roi_form(m_cmm, m_nwires[0], m_nwires[1], m_nwires[2], m_nticks, m_th_factor_ind, m_th_factor_col,
                           ...
                           m_l_jump_one_bin);
    ROI_refinement roi_refine(m_cmm, m_nwires[0], m_nwires[1], m_nwires[2], m_r_th_factor, m_r_fake_signal_low_th,
                              m_r_fake_signal_high_th, m_r_fake_signal_low_th_ind_factor,
                              m_r_fake_signal_high_th_ind_factor, m_r_pad, m_r_break_roi_loop, m_r_th_peak,
                              m_r_sep_peak, m_r_low_peak_sep_threshold_pre, m_r_max_npeaks, m_r_sigma, m_r_th_percent,
                              m_isWrapped);  //
