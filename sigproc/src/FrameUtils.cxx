...
void wct::sigproc::dump_frame(WireCell::IFrame::pointer frame)
{
    ...
    for (auto trace : *traces)
    {
        ...
        for (int ind = 0; ind < nsamps; ++ind)
        {
            ...
            if (std::isnan(val))
            {
                std::cerr << "Frame: channel " << trace->channel() << " sample " << ind
                          << " is NaN\n";
            }
            if (std::isinf(val))
            {
    ...
    std::cerr << "Frame: " << ntraces << " traces,"
              << " <mean>=" << meanmean << " TotRMS=" << totrms
              << " <tbin>=" << meantbin << " <len>=" << meanlen << std::endl;
                          ...
                          const std::vector<int> &channels)
{
    ...
    for (auto trace : traces)
    {
        ...
        if (tbin >= ncols)
        {              // underflow impossible as they are unsigned.
...
int wct::sigproc::maxcount_baseline(const ITrace::vector &traces,
                                    const WireCell::Binning &binning)
{
...
ITrace::vector wct::sigproc::tagged_traces(IFrame::pointer frame,
                                           IFrame::tag_t tag)
{
