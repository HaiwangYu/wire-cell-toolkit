...
WIRECELL_FACTORY(pdOneChannelNoise,
                 ...
                 WireCell::IChannelFilter, WireCell::IConfigurable)
WIRECELL_FACTORY(pdRelGainCalib, WireCell::SigProc::Protodune::RelGainCalib,
                 WireCell::IChannelFilter, WireCell::IConfigurable)

using namespace WireCell::SigProc;

int LedgeIdentify1(WireCell::Waveform::realseq_t &signal, double baseline,
                   int LedgeStart[3], int LedgeEnd[3])
{
    ...
    {  // find three ledges
        if (LE > 0 && StartOfLastLedgeCandidate == 0)
            break;  // no ledge candidate in the last round search
        if (StartOfLastLedgeCandidate > nticks - 200)
            break;  // the last candidate is too late in the window
        if (NumberOfLedge > 0 && (LedgeEnd[NumberOfLedge - 1] + 200) > nticks)
            break;  // the end last ledge reaches the end of readout window
        int ledge = 0;
        ...
        {
            if (NumberOfLedge ==
                0)  // no ledge found. start from 200 ticks after the last candidate
                StartWindow = (StartOfLastLedgeCandidate + 200) / UNIT;
            else
            {
                if (StartOfLastLedgeCandidate >
                    LedgeEnd[NumberOfLedge -
                             1])  // the last candidate is not a real ledge
                    // StartWindow = (StartOfLastLedgeCandidate+200)/UNIT;
        ...
        for (int i = StartWindow + 1; i < up - 1; i++)
        {
            ...
            {
                if (averaged.at(i + 1) < averaged.at(i - 1) && tolerence < JUMPS &&
                    decreaseD >
                        0)
                {  // we can ignore several jumps in the decreasing region
                    decreaseD += 2;
                    tolerence++;
                    i = i + 1;
                }
        ...
        //   if(averaged.at(i)>averaged.at(i-1)&&averaged.at(i-1)>averaged.at(i-2)){
        //   // find a edge
        //           edge = 1;
        //   }
        //   if(edge == 0) ledge = 0; // if no edge, this is not ledge
        //   if((averaged.at(i)-averaged.at(i-2)<10*UNIT)&&(averaged.at(i)-averaged.at(i-3)<10*UNIT))
        //   // slope cut
        //           ledge = 0;
        //   if(averaged.at(LedgeStart/UNIT)-baseline*UNIT>300*UNIT) ledge = 0; //
        //   ledge is close to the baseline
        // }
        // determine the end of ledge
        // case 1: find a jump of 5 ADC in the rebinned waveform
        // case 2: a continuous 20 ticks has an average close to baseline, and a RMS
        // larger than 3 ADC case 3: reaching the tick 6000 but neither 1 nor 2
        // occurs
        int tempLedgeEnd = 0;
        if (ledge == 1)
        {
            for (int i = StartOfLastLedgeCandidate / UNIT; i < up - 1;
                 i++)
            {  // case 1
                if (averaged.at(i + 1) - averaged.at(i) > 5 * UNIT)
                {
                    tempLedgeEnd = i * UNIT + 5;
                    if (tempLedgeEnd > nticks)
                        tempLedgeEnd = nticks - 1;
                    break;
                }
            }

            if (tempLedgeEnd == 0)
            {  // not find a jump, case 2
        ...
        {
            double height =
                signal.at(StartOfLastLedgeCandidate + 1) - signal.at(tempLedgeEnd);
            if (height < 0)
                ledge = 0;  // not a ledge
            if ((tempLedgeEnd - StartOfLastLedgeCandidate) >
                80)
            {  // test the decay if the ledge length is longenough
                double height50 = 0;
                height50 = signal.at(StartOfLastLedgeCandidate + 51);
                double height50Pre =
                    signal.at(StartOfLastLedgeCandidate + 1) -
                    height * (1 - exp(-50 / 100.));  // minimum 100 ticks decay time
                // if the measured is much smaller than the predicted, this is not ledge
                if (height50 - height50Pre < -12 /*-8*/)
                    ledge = 0;
        ...
        // // slope cut
        // //         ledge = 0;
        // // if(averaged.at(StartOfLastLedgeCandidate/UNIT)-baseline*UNIT>150*UNIT)
        // ledge = 0; // ledge is close to the baseline

        // // if(signal.at(tempLedgeEnd) - baseline > 100) ledge=0; // [wgu] ledge
        // end is close to the baseline
        ...
        // " <<  averaged.at(StartOfLastLedgeCandidate/UNIT)-baseline*UNIT <<
        // std::endl;
        // }

        if (ledge == 1)
        {  // ledge is close to the baseline
            if (averaged.at(tempLedgeEnd / UNIT) - baseline * UNIT > 5. * UNIT)
                ledge = 0;

            if (averaged.at(StartOfLastLedgeCandidate / UNIT) - baseline * UNIT >
                100 * UNIT)
                ledge = 0;
        }

        if (ledge == 1 &&
            StartOfLastLedgeCandidate > 1000)
        {  // ledge always follows a pulse
            // std::cerr << "[wgu] ch: " << m_ch << " StartOfLastLedgeCandidate: " <<
            // StartOfLastLedgeCandidate << std::endl;
            float hmax = 0;
            for (int i = StartOfLastLedgeCandidate - 1000;
                 i < StartOfLastLedgeCandidate - 100; i++)
            {
                if (signal.at(i) > hmax)
                    hmax = signal.at(i);
            }
            if (hmax - baseline < 200)
                ledge = 0;  // no large pre-signal
        ...
        if (ledge == 1)
        {
            ...
            LedgeEnd[NumberOfLedge] = std::min(
                tempLedgeEnd + 10, (int) signal.size());  // FIXME: ends at a threshold
            NumberOfLedge++;
        }
    }  // LE
...
bool LedgeIdentify(WireCell::Waveform::realseq_t &signal /*TH1F* h2*/,
                   double baseline, int &LedgeStart, int &LedgeEnd)
{
    int ledge = false;
    int UNIT = 10;                  // 5;    // rebin unit
    int CONTIN = 10;                // 20; // length of the continuous region
    ...
    if (ledge && LedgeStart > 20)
    {
        double height = 0;
        if (LedgeStart < 5750)
        {  // calculate the height of edge
            // double tempHeight = h2 ->GetBinContent(LedgeStart+1+200) +  h2
            // ->GetBinContent(LedgeStart+1+220) +  h2
            // ->GetBinContent(LedgeStart+1+180) +  h2
            // ->GetBinContent(LedgeStart+1+240); height = h2
            // ->GetBinContent(LedgeStart+1) - tempHeight/4;
            double tempHeight =
                signal.at(LedgeStart + 200) + signal.at(LedgeStart + 220) +
                signal.at(LedgeStart + 180) + signal.at(LedgeStart + 240);
            height = signal.at(LedgeStart) - tempHeight / 4;
            height /= 0.7;
        }
        // else height =  h2 ->GetBinContent(LedgeStart+1) -  h2
        // ->GetBinContent(6000);
        else
            height = signal.at(LedgeStart) - signal.back();
        if (height < 0)
            height = 80;  // norminal value
        if (height > 30 &&
            LedgeStart < 5900)
        {  // test the decay with a relatively large height
            double height50 = 0, height100 = 0;
            // height50 =  h2 ->GetBinContent(LedgeStart+51);
            // height100 =  h2 ->GetBinContent(LedgeStart+101);
            // double height50Pre =   h2 ->GetBinContent(LedgeStart+1)-
            // height*(1-exp(-50/100.)); // minimum 100 ticks decay time double
            // height100Pre =   h2 ->GetBinContent(LedgeStart+1) -
            // height*(1-exp(-100./100)); // minimum 100 ticks decay time

            height50 = signal.at(LedgeStart + 50);
            height100 = signal.at(LedgeStart + 100);
            double height50Pre =
                signal.at(LedgeStart) -
                height * (1 - std::exp(-50 / 100.));  // minimum 100 ticks decay time
            double height100Pre =
                signal.at(LedgeStart) -
                height * (1 - std::exp(-100. / 100));  // minimum 100 ticks decay time
            // if the measured is much smaller than the predicted, this is not ledge
            if (height50 - height50Pre < -8)
                ledge = false;
            if (height100 - height100Pre < -8)
                ledge = false;
        }
    }
    ...
    // case 2: a continuous 20 ticks has an average close to baseline, and a RMS
    // larger than 3 ADC case 3: reaching the tick 6000 but neither 1 nor 2 occurs
    if (ledge)
    {
        LedgeEnd = 0;
        for (int i = LedgeStart / UNIT; i < up - 1; i++)
        {  // case 1
...
// adapted from WCP
// int judgePlateau(int channel, TH1F* h2,double baseline, double &
// PlateauStart, double & PlateauStartEnd){
//         int continueN = 0;
//         int threshold = 200;
//         int maximumF  = 50;
...
//                                 if(
//                                 h2->GetBinContent(k+1)<baseline+threshold){
//                                         PlateauStartEnd = k-1;
//                                         break;
//                                 }
//                         }
                                   ...
                                   float stky_sig_like_rms)
{
    WireCell::Waveform::BinRangeList
        trivial_range_list;  // some ranges will be removed
    ...
    for (auto const &rng : rng_list)
    {
        ...
        if (start >= 0 && end < nsiglen)
        {
            ...
            if (rng.second == rng.first)
                isSignalLike = true;  // single sticky, do FFT interp later
            else
            {
            ...
            if (!isSignalLike)
            {  // only apply linear interpolation on non-signal-like
                // region
                for (int i = start + 1; i < end; i++)
                {
                    double content = start_content + (end_content - start_content) /
                                                         (end - start) * (i - start);
                    signal.at(i) = content;
                }
        ...
        else if (start < 0 && end < nsiglen)
        {  // sticky at the first tick
            for (int i = 0; i < end; i++)
            {
    ...
    for (auto &elem : trivial_range_list)
    {
        ...
        rng_list.erase(std::remove(rng_list.begin(), rng_list.end(), elem),
                       rng_list.end());
    }
    return true;
}
                                ...
                                WireCell::Waveform::BinRangeList &rng_list)
{
    ...
    {
        if (j % 2 == 0)
            signal_even.at(j / 2) = signal.at(j);
        else
            signal_odd.at((j - 1) / 2) = signal.at(j);
    }
    ...
    if (nsublen % 2 == 0)
    {
        std::rotate(tran_even.begin() + nsublen / 2, tran_even.begin() + nsublen,
                    tran_even.end());
    }
    else
    {
        std::rotate(tran_even.begin() + (nsublen + 1) / 2,
                    tran_even.begin() + nsublen, tran_even.end());
    }
    // inverse FFT
    ...
    if (nsublen2 % 2 == 0)
    {
        std::rotate(tran_odd.begin() + nsublen2 / 2, tran_odd.begin() + nsublen2,
                    tran_odd.end());
    }
    else
    {
        std::rotate(tran_odd.begin() + (nsublen2 + 1) / 2,
                    tran_odd.begin() + nsublen2, tran_odd.end());
    }
        ...
        if (start >= 0 && end <= nsiglen)
        {
            for (int i = start + 1; i < end; i++)
            {
                ...
                {  // predict "even" with "odd"
                    ...
                    if (ind >= 0 && ind < nsublen2)
                        signal.at(i) = signal_odd_fc.at(ind);
                    // std::cerr << "lc:fc= " << signal_lc.at(i) << " " <<
                    // signel_even_fc.at() << std::endl;
                }

                else
                {
                    int ind = i / 2. - toffset;
                    if (ind >= 0 && ind < nsublen)
                        signal.at(i) = signal_even_fc.at(ind);
                }
            }
        }
    }
...
bool Protodune::FftScaling(WireCell::Waveform::realseq_t &signal,
                           int nsamples)
{
    const int nsiglen = signal.size();
    ...
    if (nsiglen % 2 == 0)
    {  // ref test_zero_padding.cxx
        std::rotate(tran.begin() + nsiglen / 2, tran.begin() + nsiglen, tran.end());
    }
    else
    {
        std::rotate(tran.begin() + (nsiglen + 1) / 2, tran.begin() + nsiglen,
                    tran.end());
    }
    // inverse FFT
 ...
 */
Protodune::ConfigFilterBase::ConfigFilterBase(const std::string &anode,
                                              const std::string &noisedb)
  : m_anode_tn(anode)
  , m_noisedb_tn(noisedb)
{
}
Protodune::ConfigFilterBase::~ConfigFilterBase() {}
void Protodune::ConfigFilterBase::configure(
...
Protodune::ConfigFilterBase::default_configuration() const
{
    Configuration cfg;
...
Protodune::StickyCodeMitig::StickyCodeMitig(const std::string &anode,
                                            const std::string &noisedb,
                                            float stky_sig_like_val,
                                            float stky_sig_like_rms,
                                            int stky_max_len)
    ...
    if (!jext.isNull())
    {
        for (auto jone : jext)
        {
            ...
            for (auto jchan : jchans)
            {
                ...
                // std::cerr << "[wgu] ch# " << channel << " has " <<
                // jone["bits"].size() << " extra stky bits:" << std::endl;
...
WireCell::Configuration
Protodune::StickyCodeMitig::default_configuration() const
{
    Configuration cfg;
    cfg["anode"] = m_anode_tn;
    cfg["noisedb"] = m_noisedb_tn;
...
Protodune::StickyCodeMitig::apply(int ch, signal_t &signal) const
{
    ...
    // std::cerr << "[wgu] ch: " << ch << " long_stkylen: " << long_stkylen <<
    // std::endl;

    // auto signal_lc = signal; // copy, need to keep original signal
    LinearInterpSticky(signal, sticky_rng_list, m_stky_sig_like_val,
                       m_stky_sig_like_rms);
    FftInterpSticky(signal, sticky_rng_list);
    // FftShiftSticky(signal_lc, 0.5, st_ranges); // alternative approach, shift
    // by 0.5 tick signal = signal_lc;
    ...
    if (iplane == 2)
    {
        ...
        for (int LE = 0; LE < nledge; LE++)
        {
            ...
            if (overlap < 0.5 * (ledge_end[LE] - ledge_start[LE]))
            {
                ...
                ret["ledge"][ch].push_back(
                    ledge_bins);  // FIXME: maybe collection plane only?
                                  // std::cerr << "[wgu] ledge found, ch "<< ch << " , bins= [" <<
                                  // ledge_bins.first << " , " << ledge_bins.second << " ], sticky
                                  // overlap: " << overlap/(ledge_end[LE]-ledge_start[LE]) << std::endl;
            }
        }
    }
...
WireCell::Waveform::ChannelMaskMap
Protodune::StickyCodeMitig::apply(channel_signals_t &chansig) const
{
    return WireCell::Waveform::ChannelMaskMap();
}

Protodune::OneChannelNoise::OneChannelNoise(const std::string &anode,
                                            const std::string &noisedb)
...
WireCell::Configuration
Protodune::OneChannelNoise::default_configuration() const
{
    Configuration cfg;
    cfg["anode"] = m_anode_tn;
    cfg["noisedb"] = m_noisedb_tn;
...
WireCell::Waveform::ChannelMaskMap
Protodune::OneChannelNoise::apply(int ch, signal_t &signal) const
{
    WireCell::Waveform::ChannelMaskMap ret;
    ...
    if (it != m_resmp.end())
    {
        ...
        // std::cerr << "[wgu] ch: " << ch << " smpin: " << smpin << " smpout: " <<
        // smpout << std::endl;
    }
    ...
    {
        auto const &spec =
            m_noisedb->rcrc(ch);  // rc_layers set to 1 in channel noise db
    ...
    if (iplane == 2)
    {
        ...
        mag.at(0) = 0;
        Microboone::RawAdapativeBaselineAlg(
            mag);  // subtract "linear" background in spectrum
        ...
        for (int i = 0; i < (int) spec.size(); i++)
        {
            ...
            // if(i<3000) std::cout << "[wgu] spec # " << i << " : " << flag_re <<
            // std::endl;
            if (flag_re < 0.5)
            {  // found a tagged bin
                if (freqBins.empty())
        ...
        //     std::cout << "[wgu] hibin: " << br.second << " lobin: " << br.first
        //     << std::endl;
        // }
        ...
        for (int iter = 0; iter < 5; iter++)
        {
            ...
            for (auto bin : freqBins)
            {
                ...
                WireCell::Waveform::realseq_t mag_slice(
                    nslice);  // slice of magnitude spectrum
                std::copy(mag.begin() + istart, mag.begin() + iend, mag_slice.begin());
                std::pair<double, double> stat =
                    WireCell::Waveform::mean_rms(mag_slice);
                // std::cerr << "[wgu] mean/rms: " << stat.first << " " << stat.second
                // << std::endl;
                ...
                //         spectrum.at(6000+1-tbin).real(0); // FIXME: assuming 6000
                //         ticks spectrum.at(6000+1-tbin).imag(0);
                //         // std::cerr << "[wgu] chan: " << ch << " , freq tick: " <<
                //         tbin << " , amp: " << content << std::endl;
                //         }
                //     }
    ...
    // if(ch==14431) std::cerr << "[wgu] channel: " << ch << " rms: " <<
    // temp.second << std::endl; if(temp.second<min_rms || temp.second>max_rms){
    //     WireCell::Waveform::BinRange temp_chirped_bins;
    //     temp_chirped_bins.first = 0;
    //     temp_chirped_bins.second = signal.size();
...
WireCell::Waveform::ChannelMaskMap
Protodune::OneChannelNoise::apply(channel_signals_t &chansig) const
{
    return WireCell::Waveform::ChannelMaskMap();
}

Protodune::RelGainCalib::RelGainCalib(const std::string &anode,
                                      const std::string &noisedb,
                                      float gain_def, float gain_min_cut,
                                      float gain_max_cut)
  : m_anode_tn(anode)
  , m_noisedb_tn(noisedb)
  , m_gain_def(gain_def)
...
WireCell::Waveform::ChannelMaskMap
Protodune::RelGainCalib::apply(int ch, signal_t &signal) const
{
    WireCell::Waveform::ChannelMaskMap ret;

    size_t nsiglen = signal.size();
    // FIXME: already subtracted in OneChannelNoise::apply(). Need to calculate
    // again? std::pair<double,double> temp =
    // WireCell::Waveform::mean_rms(signal); auto temp_signal = signal; for
    // (size_t i=0;i!=temp_signal.size();i++){ if
    // (fabs(temp_signal.at(i)-temp.first)>6*temp.second){
    //     temp_signal.at(i) = temp.first;
    // }
    // }
    // float baseline = WireCell::Waveform::median_binned(temp_signal);
    ...
    {
        // std::cerr << "[wgu] gain_max_cut: " << m_gain_max_cut << " gain_min_cut:
        // " << m_gain_min_cut << std::endl; std::cerr << "[wgu] ch: " << ch << "
        // relative gain: " << gain_fac << std::endl;
        gain_fac = m_gain_def;
        isDefVal = true;
    }
...
WireCell::Waveform::ChannelMaskMap
Protodune::RelGainCalib::apply(channel_signals_t &chansig) const
{
    return WireCell::Waveform::ChannelMaskMap();
}
