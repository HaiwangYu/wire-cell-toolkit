...
ROI_formation::ROI_formation(Waveform::ChannelMaskMap &cmm, int nwire_u,
                             int nwire_v, int nwire_w, int nbins,
                             float th_factor_ind, float th_factor_col, int pad,
                             float asy, int rebin, double l_factor,
                             double l_max_th, double l_factor1,
                             int l_short_length, int l_jump_one_bin)
  : nwire_u(nwire_u)
  , nwire_v(nwire_v)
  , nwire_w(nwire_w)
  , nbins(nbins)
            ...
            for (size_t j = 0; j != self_rois_u.at(i).size(); j++)
            {
                ...
                for (size_t k = 0; k != self_rois_u.at(i + 2).size(); k++)
                {
                    ...
                    if (fabs(length2 - length1) < (length2 + length1) * asy)
                    {
                        ...
                        if (start3 < end3 && start3 <= end1 && start3 <= end2 &&
                            end3 >= start1 && end3 >= start2)
                        {
...
void ROI_formation::find_ROI_by_decon_itself(
    int plane, const Array::array_xxf &r_data,
    const Array::array_xxf &r_data_tight)
{
    ...
    for (int irow = 0; irow != r_data.rows(); irow++)
    {
        ...
        //  std::cout << plane << " " << signal.size() << " " << irow << " " << rms
        //  << std::endl;
        ...
        for (int j = 0; j < int(signal1.size()) - 1; j++)
        {
            ...
            if (content > threshold || (content_tight > threshold))
            {
                ...
                if (temp_roi_end > int(signal1.size()) - 1)
                    temp_roi_end = int(signal1.size()) - 1;

                //	if (abs(irow-1199)<=1&&plane==0) std::cout << "Tight: " << irow
                //<< " " << temp_roi_begin << " " << temp_roi_end << std::endl;
        ...
        // if (plane==2 && irow == 69){
        //   std::cout << "Xin: " << irow << " " << rms << " " << temp_rois.size()
        //   << std::endl; for (size_t i=0;i!=temp_rois.size();i++){
        // 	std::cout << "Xin: " << temp_rois.at(i).first << " " <<
        // temp_rois.at(i).second << std::endl;
        //   }
        // }
        ...
        //    std::cout << plane << " " << irow << " " << temp_rois.size() <<
        //    std::endl;
    }
    ...
    //   std::cout << "Xin: " << self_rois_w.at(69).at(i).first << " " <<
    //   self_rois_w.at(69).at(i).second << std::endl;
    // }
...
double ROI_formation::local_ave(Waveform::realseq_t &signal, int bin,
                                int width)
{
    double sum1 = 0;
    double sum2 = 0;
                                ...
                                int jump_one_bin)
{
    ...
    while (local_ave(signal, end + 1, 1) < local_ave(signal, end, 1) + 25 ||
           (jump_one_bin && local_ave(signal, end + 2, 1) <
                                local_ave(signal, end, 1) + 25))
    {  // ||
        //	(local_ave(signal,end+1,1) + local_ave(signal,end+2,1))*0.5 <
        //local_ave(signal,end,1) ){
        end++;
        if (end >= int(signal.size()) - 1)
        {
            end = int(signal.size()) - 1;
...
int ROI_formation::find_ROI_begin(Waveform::realseq_t &signal, int bin,
                                  double th, int jump_one_bin)
{
    ...
    while (local_ave(signal, begin - 1, 1) < local_ave(signal, begin, 1) + 25 ||
           (jump_one_bin && local_ave(signal, begin - 2, 1) <
                                local_ave(signal, begin, 1) + 25))
    {  // ||
        // (local_ave(signal,begin-2,1) + local_ave(signal,begin-1,1))*0.5 <
        // local_ave(signal,begin,1) ){
        begin--;
        if (begin <= 0)
        {
            begin = 0;
...
void ROI_formation::find_ROI_loose(int plane, const Array::array_xxf &r_data)
{
    ...
    for (int irow = 0; irow != r_data.rows(); irow++)
    {
        ...
        // std::cout << "xin3" << " " << signal.size() << " " << signal2.size() <<
        // std::endl;

        // calculate rms ...
        float th = cal_RMS(signal) * rebin * l_factor;
        // if (irow==1240) std::cout << "a " << l_factor << " " << rebin << " " <<
        // th << " " << l_max_th << std::endl;
        if (th > l_max_th)
            th = l_max_th;
        ...
        // 	std::cout << j << " " << signal2.at(j) << " " << th << " " << l_factor1
        // << " " << std::endl;
        //   }
        // }
        ...
        for (int j = 1; j < ntime - 1; j++)
        {
            ...
            if (content > th)
            {
                ...
                max_bin = begin;
                //	if (irow==1240) std::cout << "a: " << begin << " " << end << " "
                //<< j << std::endl;
            ...
            else
            {
                if (content > prev_content && content > next_content)
                {
                    ...
                    if (signal2.at(max_bin) - signal2.at(begin) + signal2.at(max_bin) -
                            signal2.at(end) >
                        th * 2)
                    {
                        flag_ROI = 1;
                    ...
                    // if (irow==1200 && plane==0)
                    //   std::cout << j << " " << begin << " " << end << " " << max_bin <<
                    //   " " << signal2.at(max_bin) * 2 - signal2.at(begin) -
                    //   signal2.at(end) << " " << th*2 << " " << temp_begin << " " <<
                    //   temp_end << " " << signal2.at(max_bin) - signal2.at(temp_begin)
                    //   << " " << signal2.at(max_bin) - signal2.at(temp_end) << " " <<
                    //   th*l_factor1 << " " << flag_ROI << std::endl;
                }
            }
            ...
            {
                // if (irow==1240) {
                //   std::cout << begin << " " << end << " " << j << " " << content << "
                //   " << th << " " << prev_content << " " << next_content << std::endl;
                //   for (int kk = begin; kk!=end; kk++){
                //     std::cout << kk << " a " << signal2.at(kk) << " " << th*l_factor1
                //     << " " << local_ave(signal2,kk,1) << std::endl; for (int
                //     kkk=0;kkk!=6;kkk++){
        ...
        //    if (irow ==1240) std::cout << "b " << begin << " " << end <<
        //    std::endl;
        //  }
            ...
            while (flag_repeat)
            {
                ...
                for (int k = 0; k < int(ROIs_1.size() - 1); k++)
                {
                    ...
                    for (int j = begin_1; j <= end_1; j++)
                    {
                        ...
                        double content =
                            current_content - ((end_content - begin_content) *
                                                   (j * 1. - begin) / (end - begin * 1.) +
                                               begin_content);
        ...
        for (int j = 0; j != int(ROIs_1.size()); j++)
        {
            ...
            // if (abs(irow-1199)<=1&& plane==0) std::cout << "Loose: "  << irow << "
            // " << ROIs_1.at(j).first << " " << ROIs_1.at(j).second << std::endl;
        }
