...
OmnibusPMTNoiseFilter::OmnibusPMTNoiseFilter(
    const std::string anode_tn, int pad_window, int min_window_length,
                                       ...
                                       output_pointer &out)
{
    ...
    for (auto cs : bychan_coll)
    {
        ...
        if (by_chan_rms[cs.first] > m_rms_threshold)
            IDPMTSignalCollection(bychan_coll[cs.first], by_chan_rms[cs.first],
                                  cs.first);
    }
    ...
    for (auto cs : bychan_indu)
    {
        // ignore dead channels ...
        if (by_chan_rms[cs.first] > m_rms_threshold)
            IDPMTSignalInduction(bychan_indu[cs.first], by_chan_rms[cs.first],
                                 cs.first, 0);
    }
    for (auto cs : bychan_indv)
    {
        // ignore dead channels ...
        if (by_chan_rms[cs.first] > m_rms_threshold)
            IDPMTSignalInduction(bychan_indv[cs.first], by_chan_rms[cs.first],
                                 cs.first, 1);
    }
    ...
    for (int i = 0; i != int(PMT_ROIs.size()); i++)
    {
        ...
        // int flag_qx = 0;
        if (PMT_ROIs.at(i)->get_sorted_uwires().size() > 0 &&
            PMT_ROIs.at(i)->get_sorted_vwires().size() > 0)
        {
            for (int j = 0; j != int(PMT_ROIs.at(i)->get_sorted_uwires().size());
                 j++)
            {
                if (PMT_ROIs.at(i)->get_average_uwires_peak_height(j) <
                    ...
                    PMT_ROIs.at(i)->get_sorted_uwires().at(j).size() <=
                        PMT_ROIs.at(i)->get_sorted_wwires().size())
                {
                    //	  flag_qx = 1;
                    for (int k = 0;
                         k != int(PMT_ROIs.at(i)->get_sorted_uwires().at(j).size());
                         k++)
                    {
                        RemovePMTSignal(
                            bychan_indu[PMT_ROIs.at(i)->get_sorted_uwires().at(j).at(k)],
                    ...
                    // << PMT_ROIs.at(i)->get_peaks().size() << " " <<
                    // PMT_ROIs.at(i)->get_sorted_uwires().at(j).size() << " " << j << " "
                    // << PMT_ROIs.at(i)->get_average_uwires_peak_height(j) << " " <<
            ...
            for (int j = 0; j != int(PMT_ROIs.at(i)->get_sorted_vwires().size());
                 j++)
            {
                        ...
                        PMT_ROIs.at(i)->get_sorted_wwires().size())
                {
                    // flag_qx = 1;
                    for (int k = 0;
                         k != int(PMT_ROIs.at(i)->get_sorted_vwires().at(j).size());
                         k++)
                    {
                        RemovePMTSignal(
                            bychan_indv[PMT_ROIs.at(i)->get_sorted_vwires().at(j).at(k)],
                            PMT_ROIs.at(i)->get_start_bin(), PMT_ROIs.at(i)->get_end_bin());
                    ...
                    //  std::cout << i << " " <<  PMT_ROIs.at(i)->get_peaks().at(0) << " "
                    //  << PMT_ROIs.at(i)->get_peaks().size() << " " <<
                    //  PMT_ROIs.at(i)->get_sorted_vwires().at(j).size() << " " << j << "
                    //  " << PMT_ROIs.at(i)->get_average_vwires_peak_height(j) << " " <<
                    //  PMT_ROIs.at(i)->get_average_wwires_peak_height() << " " <<
                    //  PMT_ROIs.at(i)->get_max_vwires_peak_height(j) << " " <<
                    //  PMT_ROIs.at(i)->get_max_wwires_peak_height() << " " <<
                    //  PMT_ROIs.at(i)->get_sorted_vwires().at(j).size() << " " <<
            ...
            if (int(PMT_ROIs.at(i)->get_sorted_wwires().size()) >=
                m_nwire_pmt_col_th)
            {
                for (int j = 0; j != int(PMT_ROIs.at(i)->get_sorted_wwires().size());
                     j++)
                {
                    RemovePMTSignal(
                        bychan_coll[PMT_ROIs.at(i)->get_sorted_wwires().at(j)],
                        PMT_ROIs.at(i)->get_start_bin(), PMT_ROIs.at(i)->get_end_bin(),
                        1);
                }
            }

            // if (flag_qx == 1) std::cout << i << " " <<
            // PMT_ROIs.at(i)->get_peaks().at(0) << " " <<
            // PMT_ROIs.at(i)->get_peaks().size() << " " <<
            // PMT_ROIs.at(i)->get_uwires().size() << " " <<
            // PMT_ROIs.at(i)->get_vwires().size() << " " <<
            // PMT_ROIs.at(i)->get_sorted_uwires().size() << " " <<
            // PMT_ROIs.at(i)->get_sorted_vwires().size() << " " <<
            // PMT_ROIs.at(i)->get_sorted_wwires().size() << " " <<
            // PMT_ROIs.at(i)->get_average_uwires_peak_height() << " " <<
            // PMT_ROIs.at(i)->get_average_vwires_peak_height() << " " <<
            // PMT_ROIs.at(i)->get_average_wwires_peak_height() << " " <<
            // PMT_ROIs.at(i)->get_max_uwires_peak_height() << " " <<
            // PMT_ROIs.at(i)->get_max_vwires_peak_height() << " " <<
            // PMT_ROIs.at(i)->get_max_wwires_peak_height() << std::endl;
        }
    ...
    SimpleFrame *sframe = new SimpleFrame(in->ident(), in->time(), itraces,
                                          in->tick(), in->masks());
                                                  ...
                                                  double rms, int ch)
{
    ...
    //  std::cout << m_threshold << " " << m_pad_window << " " <<
    //  m_min_window_length << " " << rms << " " << ch << std::endl;
        ...
        else
        {
            if (content >= -m_threshold * rms)
            {
                ...
                if (end_bin > start_bin + m_min_window_length)
                {
                    ...
                    PMTNoiseROI *ROI = new PMTNoiseROI(start_bin, end_bin, peak_bin, ch,
                                                       signal.at(peak_bin));
                    ...
                    //    float content = start_content + (end_content - start_content) *
                    //    (j - start_bin) / (end_bin - start_bin*1.0);
                    //    signal.at(j)=content;
                    //  }
                }
            }
    ...
    for (int i = 0; i != int(PMT_ROIs.size()); i++)
    {
        ...
        for (int j = 0; j != int(ROI->get_peaks().size()); j++)
        {
            ...
            if (peak_m3 < 0)
                peak_m3 = 0;
            int peak_p1 = peak + 1;
            if (peak_p1 >= int(signal.size()))
                peak_p1 = int(signal.size()) - 1;
            int peak_p2 = peak + 2;
            if (peak_p2 >= int(signal.size()))
                peak_p2 = int(signal.size()) - 1;
            int peak_p3 = peak + 3;
            if (peak_p3 >= int(signal.size()))
                peak_p3 = int(signal.size()) - 1;

            if (fabs(signal.at(peak)) > m_threshold * rms &&
                fabs(signal.at(peak)) + fabs(signal.at(peak_m1)) +
                        fabs(signal.at(peak_p1)) >
                    fabs(signal.at(peak_m1)) + fabs(signal.at(peak_m2)) +
                        fabs(signal.at(peak)) &&
                fabs(signal.at(peak)) + fabs(signal.at(peak_m1)) +
                        fabs(signal.at(peak_p1)) >
                    fabs(signal.at(peak_p1)) + fabs(signal.at(peak_p2)) +
                        fabs(signal.at(peak)) &&
                fabs(signal.at(peak)) + fabs(signal.at(peak_m1)) +
                        fabs(signal.at(peak_p1)) >
                    fabs(signal.at(peak_m2)) + fabs(signal.at(peak_m3)) +
                        fabs(signal.at(peak_m1)) &&
                fabs(signal.at(peak)) + fabs(signal.at(peak_m1)) +
                        fabs(signal.at(peak_p1)) >
                    fabs(signal.at(peak_p2)) + fabs(signal.at(peak_p3)) +
                        fabs(signal.at(peak_p1)))
            {
