...
#ifdef __DEBUG__
#define LogDebug(x) \
    std::cerr << "[yuhw]: " << __LINE__ << " : " << x << std::endl
#else
#define LogDebug(x)
#endif
ROI_refinement::ROI_refinement(
    Waveform::ChannelMaskMap &cmm, int nwire_u, int nwire_v, int nwire_w,
    float th_factor, float fake_signal_low_th, float fake_signal_high_th,
    float fake_signal_low_th_ind_factor, float fake_signal_high_th_ind_factor,
    int pad, int break_roi_loop, float th_peak, float sep_peak,
    float low_peak_sep_threshold_pre, int max_npeaks, float sigma,
  ...
  , isWrapped(isWrapped)
{
    ...
    for (auto it = cmm["bad"].begin(); it != cmm["bad"].end(); it++)
    {
        ...
        {
            bad_ch_map[ch].push_back(
                std::make_pair(it->second.at(ind).first, it->second.at(ind).second));
...
void ROI_refinement::Clear()
{
    for (int i = 0; i != nwire_u; i++)
    {
        for (auto it = rois_u_tight.at(i).begin(); it != rois_u_tight.at(i).end();
             it++)
        {
            delete *it;
        }
        rois_u_tight.at(i).clear();
        for (auto it = rois_u_loose.at(i).begin(); it != rois_u_loose.at(i).end();
             it++)
        {
            delete *it;
        }
    ...
    for (int i = 0; i != nwire_v; i++)
    {
        for (auto it = rois_v_tight.at(i).begin(); it != rois_v_tight.at(i).end();
             it++)
        {
            delete *it;
        }
        rois_v_tight.at(i).clear();
        for (auto it = rois_v_loose.at(i).begin(); it != rois_v_loose.at(i).end();
             it++)
        {
            delete *it;
        }
    ...
    for (int i = 0; i != nwire_w; i++)
    {
        for (auto it = rois_w_tight.at(i).begin(); it != rois_w_tight.at(i).end();
             it++)
        {
            delete *it;
        }
...
void ROI_refinement::apply_roi(int plane, Array::array_xxf &r_data)
{
    if (plane == 0)
    {
        for (int irow = 0; irow != r_data.rows(); irow++)
        {
            ...
            for (auto it = rois_u_loose.at(irow).begin();
                 it != rois_u_loose.at(irow).end(); it++)
            {
                SignalROI *roi = *it;
                ...
                {
                    int content =
                        r_data(irow, i) - ((end_content - start_content) *
                                               (i - start_bin) / (end_bin - start_bin) +
                                           start_content);
                    signal.at(i) = content;
    ...
    else if (plane == 1)
    {
        for (int irow = 0; irow != r_data.rows(); irow++)
        {
            ...
            Waveform::realseq_t signal(r_data.cols(), 0);
            // loop ROIs and assign data
            for (auto it = rois_v_loose.at(irow).begin();
                 it != rois_v_loose.at(irow).end(); it++)
            {
                SignalROI *roi = *it;
                // int start_bin = roi->get_start_bin();
                // int end_bin = roi->get_end_bin();
    ...
    else
    {
        for (int irow = 0; irow != r_data.rows(); irow++)
        {
            ...
            // if (irow==69){
            // 	std::cout << "Xin: " << rois_w_tight.at(irow).size() << " " <<
            // std::endl; 	for (auto it = rois_w_tight.at(irow).begin(); it!=
            // rois_w_tight.at(irow).end(); it++){ 	  SignalROI *roi =  *it; 	  int
            // start_bin = roi->get_ext_start_bin(); 	  int end_bin =
            // roi->get_ext_end_bin(); 	  std::cout << "Xin: " << start_bin << " " <<
            // end_bin << std::endl;
            // 	}
            // }

            // loop ROIs and assign data
            for (auto it = rois_w_tight.at(irow).begin();
                 it != rois_w_tight.at(irow).end(); it++)
            {
                SignalROI *roi = *it;
                // int start_bin = roi->get_start_bin();
                // int end_bin = roi->get_end_bin();
                ...
                for (int i = start_bin; i < end_bin + 1; i++)
                {
                    int content = r_data(irow, i);  // - ((end_content -
                                                    // start_content)*(i-start_bin)/(end_bin-start_bin)
                                                    // + start_content);
                    signal.at(i) = content;
                }
            }
...
void ROI_refinement::load_data(int plane, const Array::array_xxf &r_data,
                               ROI_formation &roi_form)
{
    ...
    for (int irow = 0; irow != r_data.rows(); irow++)
    {
        ...
        // load tight rois
        std::vector<std::pair<int, int>> &uboone_rois =
            roi_form.get_self_rois(irow + offset);
        for (size_t i = 0; i != uboone_rois.size(); i++)
        {
            SignalROI *tight_roi =
                new SignalROI(plane, irow + offset, uboone_rois.at(i).first,
                              uboone_rois.at(i).second, signal);
            ...
            if (plane == 0)
            {
                ...
                // [Hongzhao] connectivity map for Induction Planes' tight ROIs was
                // NEVER used after initialization CHANGE(S): Hongzhao commented out the
                // initialization of connectivity map for Induction Planes' tight ROIs
                //       	if (chid>0){
                //       	  //form connectivity map
                //       	  for (auto it =
                //       rois_u_tight[chid-1].begin();it!=rois_u_tight[chid-1].end();it++){
                //       	    SignalROI *prev_roi = *it;
                //      	    if (tight_roi->overlap(prev_roi)){
                //      	      if (front_rois.find(prev_roi) ==
                //      front_rois.end()){ 		SignalROISelection temp_rois;
            ...
            else if (plane == 1)
            {
                ...
                //      nwire_u-1].begin();it!=rois_v_tight[chid-nwire_u-1].end();it++){
                //      	    SignalROI *prev_roi = *it;
                //      	    if (tight_roi->overlap(prev_roi)){
                //      	      if (front_rois.find(prev_roi) ==
                //      front_rois.end()){ 		SignalROISelection temp_rois;
            ...
            else
            {
                ...
                {
                    // form connectivity map
                    // [Hongzhao] should avoid fake adjacency in Collection Plane for
                    // wrapped configuration CHANGE(S): Hongzhao added protection to skip
                    // fake adjacency
                    if (chid != nwire_u + nwire_v + nwire_w / 2. ||
                        !isWrapped)
                    {  // additional judgement to skip fake adjacency
                ...
                // In principle, we don't need to explicitly create the connectivity map
                // for channel N & N+1 WHEN we already LOOP OVER all ROIs in channel N
                // to create the connectivity map with channel N-1 (think about the case
                // if N became N+1) BUT, if we only update one ROI, we need to consider
                // both sides (see generate_merge_ROIs()) In practical, channel N+1 here
                // is empty. CHANGE(S): Hongzhao commented out the creation of the
                // connectivity map for channel N & N+1 if
                // (chid<nwire_u+nwire_v+nwire_w-1){
                //      	  //form connectivity map
                //      	  for (auto it =
        ...
        if (plane != 2)
        {
            ...
            {
                SignalROI *loose_roi =
                    new SignalROI(plane, chid, uboone_rois.at(i).first,
                                  uboone_rois.at(i).second, signal);
                float threshold = plane_rms.at(irow) * th_factor;
                ...
                if (plane == 0)
                {
                    ...
                    {
                        // form connectivity map
                        for (auto it = rois_u_loose[chid - 1].begin();
                             it != rois_u_loose[chid - 1].end(); it++)
                        {
                            SignalROI *prev_roi = *it;
                    ...
                    // CHANGE(S): Hongzhao commented out the creation of the connectivity
                    // map for channel N & N+1
                    //  if (chid<nwire_u-1){
                    //    //form connectivity map
                    //    for (auto
                    //    it=rois_u_loose[chid+1].begin();it!=rois_u_loose[chid+1].end();it++){
                    //      SignalROI *next_roi = *it;
                    ...
                    // CHANGE(S): connect the LAST & FIRST osp_wire_number in an Induction
                    // plane
                    if (chid == nwire_u - 1 && isWrapped)
                    {
                        // form connectivity map
                        for (auto it = rois_u_loose[0].begin(); it != rois_u_loose[0].end();
                             it++)
                        {
                            SignalROI *next_roi = *it;
                            if (loose_roi->overlap(next_roi))
                    ...
                    // form contained map ...
                    for (auto it = rois_u_tight[chid].begin();
                         it != rois_u_tight[chid].end(); it++)
                    {
                        ...
                        if (tight_roi->overlap(loose_roi))
                        {
                            ...
                            // 	std::cout << tight_roi->get_start_bin() << " " <<
                            // loose_roi->get_start_bin() << " " << tight_roi->get_chid() << "
                            // " << tight_roi->get_plane() << std::endl;
                        }
                    }
                }
                else if (plane == 1)
                {
                    ...
                    {
                        // form connectivity map
                        for (auto it = rois_v_loose[0].begin(); it != rois_v_loose[0].end();
                             it++)
                        {
                            SignalROI *next_roi = *it;
                            if (loose_roi->overlap(next_roi))
                    ...
                    // form contained map ...
                    for (auto it = rois_v_tight[chid - nwire_u].begin();
                         it != rois_v_tight[chid - nwire_u].end(); it++)
                    {
                        SignalROI *tight_roi = *it;
                        if (tight_roi->overlap(loose_roi))
                        {
...
void ROI_refinement::CleanUpROIs(int plane)
{
                ...
                if (ROIsaved_map.find(roi) == ROIsaved_map.end())
                {
                    if (contained_rois.find(roi) != contained_rois.end())
                    {
                        ...
                        while (temp_rois.size())
                        {
                            ...
                            {
                                for (auto it1 = front_rois[temp_roi].begin();
                                     it1 != front_rois[temp_roi].end(); it1++)
                                {
                            ...
                            {
                                for (auto it1 = back_rois[temp_roi].begin();
                                     it1 != back_rois[temp_roi].end(); it1++)
                                {
                                    if (ROIsaved_map.find(*it1) == ROIsaved_map.end())
                                    {
                                        temp_rois.push_back(*it1);
        ...
        for (size_t i = 0; i != rois_u_loose.size(); i++)
        {
                 ...
                 it++)
            {
                ...
                if (ROIsaved_map.find(roi) == ROIsaved_map.end())
                {
                    ...
                    {
                        for (auto it1 = front_rois[roi].begin();
                             it1 != front_rois[roi].end(); it1++)
                        {
                            auto it2 =
                                find(back_rois[*it1].begin(), back_rois[*it1].end(), roi);
                    ...
                    if (back_rois.find(roi) != back_rois.end())
                    {
                        ...
                        {
                            auto it2 =
                                find(front_rois[*it1].begin(), front_rois[*it1].end(), roi);
            ...
            {
                auto it1 =
                    find(rois_u_loose.at(i).begin(), rois_u_loose.at(i).end(), *it);
    ...
    else if (plane == 1)
    {
        ...
        for (size_t i = 0; i != rois_v_loose.size(); i++)
        {
            ...
            {
                auto it1 =
                    find(rois_v_loose.at(i).begin(), rois_v_loose.at(i).end(), *it);
    ...
    // std::cout << counter << " " << ROIsaved_map.size() << " " << counter1 << "
    // " << counter2 << std::endl;
}
    ...
    if (plane == 0)
    {
        for (int i = 0; i != nwire_u; i++)
        {
                 ...
                 it++)
            {
                ...
                if (contained_rois.find(roi) != contained_rois.end())
                {
                    for (auto it1 = contained_rois[roi].begin();
                         it1 != contained_rois[roi].end(); it1++)
                    {
            ...
            // if (i == 1212)
            //   std::cout << saved_rois.size() << " " <<
            //   saved_rois.at(0)->get_start_bin() << " " <<
            ...
            for (auto it = saved_rois.begin(); it != saved_rois.end(); it++)
            {
                ...
                if (i < nwire_u - 1)
                {
                    for (auto it1 = rois_u_loose.at(i + 1).begin();
                         it1 != rois_u_loose.at(i + 1).end(); it1++)
                    {
                ...
                {
                    for (auto it1 = rois_u_loose.at(0).begin();
                         it1 != rois_u_loose.at(0).end(); it1++)
                    {
                        SignalROI *next_roi = *it1;
                ...
                if (i == 0 && isWrapped)
                {
                    for (auto it1 = rois_u_loose.at(nwire_u - 1).begin();
                         it1 != rois_u_loose.at(nwire_u - 1).end(); it1++)
                    {
                        SignalROI *prev_roi = *it1;
                        if (loose_roi->overlap(prev_roi))
                        {
    ...
    else if (plane == 1)
    {
        for (int i = 0; i != nwire_v; i++)
        {
            ...
            for (auto it = saved_rois.begin(); it != saved_rois.end(); it++)
            {
                ...
                if (i < nwire_v - 1)
                {
                    for (auto it1 = rois_v_loose.at(i + 1).begin();
                         it1 != rois_v_loose.at(i + 1).end(); it1++)
                    {
                        SignalROI *next_roi = *it1;
                ...
                {
                    for (auto it1 = rois_v_loose.at(0).begin();
                         it1 != rois_v_loose.at(0).end(); it1++)
                    {
                        SignalROI *next_roi = *it1;
                ...
                if (i == 0 && isWrapped)
                {
                    for (auto it1 = rois_v_loose.at(nwire_v - 1).begin();
                         it1 != rois_v_loose.at(nwire_v - 1).end(); it1++)
                    {
                        SignalROI *prev_roi = *it1;
                        if (loose_roi->overlap(prev_roi))
                        {
...
void ROI_refinement::CheckROIs(int plane, ROI_formation &roi_form)
{
    if (plane == 0)
    {
        ...
        //   for (auto it = rois_u_loose.at(i).begin(); it!=
        //   rois_u_loose.at(i).end();it++){
        //     SignalROI *roi = *it;
        ...
        //     if (front_rois.find(roi)!=front_rois.end()){
        //  	for (auto it1 =
        //  front_rois[roi].begin();it1!=front_rois[roi].end();it1++){
        // 	  SignalROI *roi1 = *it1;
                 ...
                 it++)
            {
                ...
                if (front_rois.find(roi) != front_rois.end())
                {
                         ...
                         it1++)
                    {
                        SignalROI *roi1 = *it1;
                        int chid1 = roi1->get_chid();
                        // std::cout << "F " << i << " " << rois_u_loose.size() << " " <<
                        // roi1 << " " << chid << " " << chid1 << std::endl;
                        float th1;
...
void ROI_refinement::CleanUpCollectionROIs()
{
    ...
    // scan with all the tight ROIs to look for peaks above certain threshold, put
    // in a temporary set
    float mean_threshold = fake_signal_low_th;
    float threshold =
        fake_signal_high_th;  // electrons, about 1/2 of MIP per tick ...
    ...
    for (int i = 0; i != nwire_w; i++)
    {
        ...
        {
            SignalROI *roi = *it;
            // std::cout << "Xin: " << roi->get_max_height() << " " <<
            // roi->get_average_heights() << std::endl;
            if (roi->get_above_threshold(threshold).size() != 0 ||
                roi->get_average_heights() > mean_threshold)
                Good_ROIs.insert(roi);
        }
    }
    // for a particular ROI if it is not in, or it is not connected with one in
    // the temporary map, then remove it
    ...
    for (int i = 0; i != nwire_w; i++)
    {
             ...
             it++)
        {
            ...
            if (Good_ROIs.find(roi) != Good_ROIs.end())
                continue;
            if (front_rois.find(roi) != front_rois.end())
            {
    ...
    for (auto it = Bad_ROIs.begin(); it != Bad_ROIs.end(); it++)
    {
        ...
        auto it1 =
            find(rois_w_tight.at(chid).begin(), rois_w_tight.at(chid).end(), roi);
...
void ROI_refinement::CleanUpInductionROIs(int plane)
{
    ...
    //    for (int i=0;i!=nwire_u;i++){
    //      for (auto it =
    //      rois_u_loose.at(i).begin();it!=rois_u_loose.at(i).end();it++){
    // SignalROI* roi = *it;
    // //std::cout << "Xin: " << roi->get_max_height() << " " <<
    // roi->get_average_heights() << std::endl; if
    // (front_rois.find(roi)==front_rois.end() &&
    // back_rois.find(roi)==back_rois.end()){
    //   if (roi->get_above_threshold(threshold).size()==0 &&
    //   roi->get_average_heights() < mean_threshold)
    //     Bad_ROIs.push_back(roi);
    // }
    ...
    //      SignalROI* roi = *it;
    //      int chid = roi->get_chid();
    //      auto it1 = find(rois_u_loose.at(chid).begin(),
    //      rois_u_loose.at(chid).end(),roi); if (it1 !=
    //      rois_u_loose.at(chid).end())
    ...
    //    for (int i=0;i!=nwire_v;i++){
    //      for (auto it =
    //      rois_v_loose.at(i).begin();it!=rois_v_loose.at(i).end();it++){
    // SignalROI* roi = *it;
    // //	std::cout << "Xin: " << roi->get_max_height() << " " <<
    // roi->get_average_heights() << std::endl; if
    // (front_rois.find(roi)==front_rois.end() &&
    ...
    //      SignalROI* roi = *it;
    //      int chid = roi->get_chid()-nwire_u;
    //      auto it1 = find(rois_v_loose.at(chid).begin(),
    //      rois_v_loose.at(chid).end(),roi); if (it1 !=
    //      rois_v_loose.at(chid).end())
    ...
    if (plane == 0)
    {
        ...
        for (auto it = Bad_ROIs.begin(); it != Bad_ROIs.end(); it++)
        {
            ...
            auto it1 =
                find(rois_u_loose.at(chid).begin(), rois_u_loose.at(chid).end(), roi);
    ...
    else if (plane == 1)
    {
        ...
        for (auto it = Bad_ROIs.begin(); it != Bad_ROIs.end(); it++)
        {
            ...
            auto it1 =
                find(rois_v_loose.at(chid).begin(), rois_v_loose.at(chid).end(), roi);
...
void ROI_refinement::ShrinkROI(SignalROI *roi, ROI_formation &roi_form)
{
    ...
    // TH1F *htemp = new
    // TH1F("htemp","htemp",end_bin-start_bin+1,start_bin,end_bin+1);

    // check tight ROIs
    if (contained_rois.find(roi) != contained_rois.end())
    {
        for (auto it = contained_rois[roi].begin(); it != contained_rois[roi].end();
             it++)
        {
            SignalROI *tight_roi = *it;
    ...
    if (front_rois.find(roi) != front_rois.end())
    {
        for (auto it = front_rois[roi].begin(); it != front_rois[roi].end(); it++)
        {
            ...
            std::vector<std::pair<int, int>> contents_above_threshold =
                next_roi->get_above_threshold(threshold);
            for (size_t i = 0; i != contents_above_threshold.size(); i++)
            {
                if (chid == channel_save && print_flag)
                    std::cout << "Front " << chid1 << " "
                              << start_bin1 + contents_above_threshold.at(i).first << " "
                              << start_bin1 + contents_above_threshold.at(i).second
                              << std::endl;

                for (int j = contents_above_threshold.at(i).first;
                     j <= contents_above_threshold.at(i).second; j++)
                {
                    if (j + start_bin1 - start_bin >= 0 &&
                        j + start_bin1 - start_bin < int(temp_signal.size()))
                    {
    ...
    if (back_rois.find(roi) != back_rois.end())
    {
        for (auto it = back_rois[roi].begin(); it != back_rois[roi].end(); it++)
        {
            ...
            std::vector<std::pair<int, int>> contents_above_threshold =
                prev_roi->get_above_threshold(threshold);
            for (size_t i = 0; i != contents_above_threshold.size(); i++)
            {
                if (chid == channel_save && print_flag)
                    std::cout << "Back " << chid1 << " "
    ...
    // }
    // for (int i=0;i!=contents.size();i++){
    //   // if (contents.at(i) > max/2. && contents.at(i) > threshold1*2 )
    ...
    new_start_bin -= pad;
    new_end_bin += pad;
    if (new_start_bin < start_bin)
        new_start_bin = start_bin;
    if (new_end_bin > end_bin)
        new_end_bin = end_bin;

    if (chid == channel_save && print_flag)
        std::cout << "check contents "
                  << " " << start_bin << " " << end_bin << " " << new_start_bin
                  << " " << new_end_bin << std::endl;
    ...
    {
        SignalROI *new_roi =
            new SignalROI(plane, chid, new_start_bin, new_end_bin, signal);
    ...
    if (chid < nwire_u)
    {
        auto it = std::find(rois_u_loose.at(chid).begin(),
                            rois_u_loose.at(chid).end(), roi);
    ...
    {
        auto it = std::find(rois_v_loose.at(chid - nwire_u).begin(),
                            rois_v_loose.at(chid - nwire_u).end(), roi);
...
void ROI_refinement::BreakROI(SignalROI *roi, float rms)
{
    ...
    {
        log->debug(
            "ROI_refinement: local ch index {} (plane {}), found max peaks "
            "{} with threshold={}",
    ...
    if (nfound > 1)
    {
        ...
        if (npeaks_threshold > 1)
        {
            ...
            if (valley_pos[0] != start_bin)
            {
                ...
                {
                    if (temp_signal.at(j - start_bin) >
                        temp_signal.at(order_peak_pos[0] - start_bin))
            ...
            for (int j = 0; j != npeaks - 1; j++)
            {
                ...
                //	std::cout << order_peak_pos[j]-start_bin << " " <<
                //order_peak_pos[j+1]-start_bin << std::endl;

                for (int k = order_peak_pos[j] - start_bin;
                     k < order_peak_pos[j + 1] - start_bin; k++)
                {
                    if (temp_signal.at(k) < min)
                    {
                        min = temp_signal.at(k);
            ...
            min = 1e9;
            for (int k = order_peak_pos[npeaks - 1] - start_bin;
                 k <= end_bin - start_bin; k++)
            {
                if (temp_signal.at(k) < min)
                {
                    min = temp_signal.at(k);
            ...
            if (valley_pos[npeaks] != end_bin)
            {
                ...
                {
                    if (temp_signal.at(j - start_bin) >
                        temp_signal.at(order_peak_pos[npeaks - 1] - start_bin))
            ...
            // 	for (int j=0;j!=npeaks;j++){
            // 	  std::cout << valley_pos[j] << " " <<
            // htemp->GetBinContent(valley_pos[j]-start_bin+1)<< " " <<
            // order_peak_pos[j] << " " << htemp->GetBinContent(
            // order_peak_pos[j]-start_bin+1) << " " << valley_pos[j+1] << " " <<
            // htemp->GetBinContent(valley_pos[j+1] - start_bin+1)<< " " << rms *
            // sep_peak << std::endl ;
            // 	}
            // }
            ...
            for (int j = 0; j < npeaks; j++)
            {
                if (npeaks1 > 0)
                {
                    // std::cout << valley_pos[j]-start_bin << " " <<
                    // valley_pos1[npeaks1]-start_bin << std::endl;
                    // find the lowest valley except the first peak, except the first one
                    if (temp_signal.at(valley_pos[j] - start_bin) <
                        temp_signal.at(valley_pos1[npeaks1] - start_bin))
                    {
                        valley_pos1[npeaks1] = valley_pos[j];
                    }
                }

                // if (roi->get_chid() == 1195 && roi->get_plane() ==
                // WirePlaneType_t(0)){
                //   std::cout << "c: " << order_peak_pos[j] << " " <<
                //   htemp->GetBinContent(order_peak_pos[j]-start_bin+1) << " " <<
                //   valley_pos1[npeaks1] << " " <<
                //   htemp->GetBinContent(valley_pos1[npeaks1]-start_bin+1) <<
                //   std::endl;
                // }

                // find the next peak
                if (temp_signal.at(order_peak_pos[j] - start_bin) -
                        temp_signal.at(valley_pos1[npeaks1] - start_bin) >
                    low_peak_sep_threshold)
                {
                    ...
                    //   std::cout << "c: " << npeaks << " " << valley_pos1[npeaks1-1] <<
                    //   " " << peak_pos1[npeaks1-1] << " " << valley_pos1[npeaks1] << " "
                    //   << rms * sep_peak << std::endl;
                }
            }
            // fill the last valley
            valley_pos1[npeaks1] = valley_pos[npeaks];
            // std::cout << roi->get_plane() << " " << roi->get_chid() << " " <<
            // npeaks << " " << npeaks1 << " " ; std::cout << start_bin << " " <<
            // end_bin << std::endl;
            ...
            for (int j = 0; j != npeaks1; j++)
            {
                LogDebug("{" << roi->get_chid() << ", " << roi->get_start_bin() << "} "
                             << valley_pos1[j] << " " << peak_pos1[j] << " "
                             << valley_pos1[j + 1] << " : " << section_protected[j]);
            }
            // }
            ...
            for (int j = 0; j != npeaks1; j++)
            {
                // int flag = 0;
                int start_pos = valley_pos1[j];
                double start_content = temp1_signal.at(
                    valley_pos1[j] -
                    start_bin);  // htemp1->GetBinContent(valley_pos1[j]-start_bin+1);
                int end_pos = valley_pos1[j + 1];
                double end_content = temp1_signal.at(
                    valley_pos1[j + 1] -
                    start_bin);  // htemp1->GetBinContent(valley_pos1[j+1]-start_bin+1);
            ...
            //  	std::cout << "Break:  "  << roi->get_chid() << " " << start_bin
            //  << " " << end_bin << " " << nfound << std::endl; 	std::cout << npeaks1
            //  << std::endl;
            // 	std::cout << contained_rois[roi].size() << std::endl;

            for (auto it = contained_rois[roi].begin();
                 it != contained_rois[roi].end(); it++)
            {
                SignalROI *temp_roi = *it;
                int temp_flag = 0;
                for (int i = temp_roi->get_start_bin(); i <= temp_roi->get_end_bin();
                     i++)
                {
                    // std::cout << i-roi->get_start_bin() << " " << i << " " <<
                    // temp_roi->get_start_bin() << " " << temp_roi->get_end_bin() << " "
                ...
                if (temp_flag == 0)
                {
                    for (int i = temp_roi->get_start_bin(); i <= temp_roi->get_end_bin();
                         i++)
                    {
                        // std::cout << i-roi->get_start_bin() << " " <<
                        // i-temp_roi->get_start_bin() << std::endl;
                        if (i - int(roi->get_start_bin()) >= 0 &&
                            i - int(roi->get_start_bin()) < int(temp_signal.size()))
    ...
    // if (roi->get_chid() == 1151){
    //   std::cout << "Break:  "  << roi->get_chid() << " " << start_bin << " " <<
    //   end_bin << " " << nfound << std::endl; for (int
    //   i=0;i<htemp->GetNbinsX();i++){
    ...
    for (int qx = 0; qx != 2; qx++)
    {
        ...
        for (int i = 0; i < int(temp_signal.size()); i++)
        {
            if (temp_signal.at(i) < th_factor * rms)
            {
                ...
                // if (roi->get_chid() == 1240)
                //   std::cout << qx << " " <<  start+start_bin << " " << end +
                //   start_bin << " " << 3*rms  << std::endl;
                i = end;
            }
        }
...
void ROI_refinement::BreakROI1(SignalROI *roi)
{
    ...
    // if (chid == 1274)
    //   std::cout << "BreakROI1: " << chid << " " << roi->get_start_bin() << " "
    //   << roi->get_end_bin() << " " << bins.size()  << " " <<
    //   htemp->GetBinContent(1) << " " <<
    //   htemp->GetBinContent(end_bin-start_bin+1) << std::endl;
    ...
    for (int i = 0; i != int(bins.size()) - 1; i++)
    {
        ...
        {
            SignalROI *sub_roi =
                new SignalROI(plane, chid, start_bin1, end_bin1, signal);
...
void ROI_refinement::BreakROIs(int plane, ROI_formation &roi_form)
{
    ...
    for (size_t i = 0; i != all_rois.size(); i++)
    {
        ...
        //   std::cout << all_rois.at(i)->get_chid() << " " <<
        //   all_rois.at(i)->get_start_bin() << " " << all_rois.at(i)->get_end_bin()
        //   << std::endl; for (int
        //   j=0;j!=all_rois.at(i)->get_contents().size();j++){
        // 	std::cout << j << " " << all_rois.at(i)->get_contents().at(j) <<
        // std::endl;
        //   }
        // }
...
{
    // if (plane==2) std::cout << "Xin: " << rois_w_tight.at(69).size() << " " <<
    // std::endl;

    // std::cout << "Clean up loose ROIs" << std::endl;
    CleanUpROIs(plane);
    // std::cout << "Generate more loose ROIs from isolated good tight ROIs" <<
    // std::endl;
...
void ROI_refinement::refine_data_debug_mode(int plane, ROI_formation &roi_form,
...
void ROI_refinement::MultiPlaneProtection(
    const int plane, const IAnodePlane::pointer anode,
    ...
    const int wire_resolution, const int nbounds_layers)
{
    ...
    for (int iplane = 0; iplane < 3; ++iplane)
    {
            ...
            for (auto roi : rois)
            {
                ...
                for (auto wire : wires)
                {
                         ...
                         tick < roi->get_end_bin() / tick_resolution; ++tick)
                    {
                        ...
                        //  LogDebug(iplane << ", " << tick*tick_resolution << ", " <<
                        //  pit_id);

                        if (roi->get_contents().at(content_id) <
                            //  threshold *
                            //  roi_form.get_rms_by_plane(iplane).at(roi->get_chid())
                            threshold)
                            continue;
        ...
        //            << " map_tick_pitch_roi: " <<
        //            map_tick_pitch_roi[iplane].size()
        //            << std::endl;
    }
        ...
        for (auto tc2 : map_tick_coord[ref_planes[1]])
        {
            ...
            for (auto c1 : tc1.second)
            {
                for (auto c2 : tc2.second)
                {
                    ...
                    //  LogDebug(tick*tick_resolution << ", " << c1.grid << ", " <<
                    //  c2.grid);
                    if (map_tick_pitch_roi[plane].find(tick) ==
                        map_tick_pitch_roi[plane].end())
                        continue;
    ...
    {
        LogDebug(" total rois:          "
                 << get_rois_by_plane(plane).size() << " at plane " << plane
...
void ROI_refinement::MultiPlaneROI(
    const int target_plane, const IAnodePlane::pointer anode,
    ...
    const int nbounds_layers)
{
                ...
                for (auto c2 : tc2.second)
                {
                         ...
                         pitch_target < index + wire_resolution; ++pitch_target)
                    {
                        std::pair<int, int> key = {
                            map_wireid_roichid[target_plane][pitch_target], 0};
                        int sta = tick * tick_resolution;
                        int end = sta + tick_resolution;
    ...
    {
        LogDebug(" total rois:          "
                 << get_rois_by_plane(target_plane).size() << " at target_plane "
...
void ROI_refinement::TestROIs()
{
    ...
    {
        for (auto it = rois_v_loose.at(chid).begin();
             it != rois_v_loose.at(chid).end(); it++)
        {
            SignalROI *roi = *it;
            // loop through front
            for (auto it1 = front_rois[roi].begin(); it1 != front_rois[roi].end();
            ...
            {
                SignalROI *roi1 = *it1;
                if (find(rois_v_loose.at(chid + 1).begin(),
                         rois_v_loose.at(chid + 1).end(),
                         roi1) == rois_v_loose.at(chid + 1).end())
                    std::cout << chid << " v " << +1 << " " << roi << " " << roi1
                              << std::endl;
            }
    ...
    {
        for (auto it = rois_w_tight.at(chid).begin();
             it != rois_w_tight.at(chid).end(); it++)
        {
            SignalROI *roi = *it;
            // loop through front
            for (auto it1 = front_rois[roi].begin(); it1 != front_rois[roi].end();
            ...
            {
                SignalROI *roi1 = *it1;
                if (find(rois_w_tight.at(chid + 1).begin(),
                         rois_w_tight.at(chid + 1).end(),
                         roi1) == rois_w_tight.at(chid + 1).end())
                    std::cout << chid << " w " << +1 << " " << roi << " " << roi1
                              << std::endl;
            }
...
void ROI_refinement::ExtendROIs(int plane)
{
    ...
    if (plane == 0)
    {
        ...
        for (int chid = 0; chid != nwire_u; chid++)
        {
                 ...
                 it != rois_u_loose.at(chid).end(); it++)
            {
                ...
                if (flag)
                {
                         ...
                         it1++)
                    {
                        ...
                        if (ext_start_bin > roi1->get_start_bin())
                            ext_start_bin = roi1->get_start_bin();
                        if (ext_end_bin < roi1->get_end_bin())
                            ext_end_bin = roi1->get_end_bin();
                        roi->set_ext_start_bin(ext_start_bin);
                        roi->set_ext_end_bin(ext_end_bin);
                        //	std::cout << roi->get_chid() << " " << roi1->get_chid() << " "
                        //<< roi->get_start_bin() << " " << roi->get_end_bin() << " " <<
                        //roi1->get_start_bin() << " " << roi1->get_end_bin() << std::endl;
                    }
                    ...
                    // std::cout << chid << " " << roi->get_start_bin() << " " <<
                    // roi->get_end_bin() << std::endl;
                }
            }
            ...
            if (flag)
            {
                     ...
                     it != rois_u_loose.at(chid).end(); it++)
                {
                    ...
                    if (prev_roi != 0)
                    {
                        ...
                        {
                            prev_roi->set_ext_end_bin(int(prev_roi->get_end_bin() * 0.5 +
                                                          roi->get_start_bin() * 0.5));
                            roi->set_ext_start_bin(int(prev_roi->get_end_bin() * 0.5 +
                                                       roi->get_start_bin() * 0.5));
                        }
                    }
                    prev_roi = roi;
            ...
            // for (auto it = rois_u_loose.at(chid).begin(); it!=
            // rois_u_loose.at(chid).end();it++){
            //   SignalROI *roi =  *it;
            //   std::cout << chid << " " << roi->get_ext_start_bin() << " " <<
            //   roi->get_ext_end_bin() << std::endl;
            // }
        }
    }
    else if (plane == 1)
    {
        ...
        for (int chid = 0; chid != nwire_v; chid++)
        {
            ...
            // for (auto it = rois_v_loose.at(chid).begin(); it!=
            // rois_v_loose.at(chid).end();it++){
            //   SignalROI *roi =  *it;
            //   std::cout << chid << " " << roi->get_ext_start_bin() << " " <<
            //   roi->get_ext_end_bin() << std::endl;
            // }
    ...
    else
    {
        ...
        for (int chid = 0; chid != nwire_w; chid++)
        {
            ...
            // for (auto it = rois_w_tight.at(chid).begin(); it!=
            // rois_w_tight.at(chid).end();it++){
            //   SignalROI *roi =  *it;
            //   std::cout << chid << " " << roi->get_ext_start_bin() << " " <<
            //   roi->get_ext_end_bin() << std::endl;
            // }
