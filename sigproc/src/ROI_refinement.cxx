...
ROI_refinement::ROI_refinement(Waveform::ChannelMaskMap &cmm, int nwire_u,
                               int nwire_v, int nwire_w, float th_factor,
                               float fake_signal_low_th,
                               float fake_signal_high_th,
                               float fake_signal_low_th_ind_factor,
                               float fake_signal_high_th_ind_factor, int pad,
                               int break_roi_loop, float th_peak,
                               float sep_peak, float low_peak_sep_threshold_pre,
                               int max_npeaks, float sigma, float th_percent)
    : nwire_u(nwire_u), nwire_v(nwire_v), nwire_w(nwire_w),
      th_factor(th_factor), fake_signal_low_th(fake_signal_low_th),
      fake_signal_high_th(fake_signal_high_th),
      fake_signal_low_th_ind_factor(fake_signal_low_th_ind_factor),
      fake_signal_high_th_ind_factor(fake_signal_high_th_ind_factor), pad(pad),
      break_roi_loop(break_roi_loop), th_peak(th_peak), sep_peak(sep_peak),
      low_peak_sep_threshold_pre(low_peak_sep_threshold_pre),
      max_npeaks(max_npeaks), sigma(sigma), th_percent(th_percent),
    ...
    for (size_t i = 0; i != uboone_rois.size(); i++) {
      ...
      if (plane == 0) {
        ...
        if (chid > 0) {
          ...
          for (auto it = rois_u_tight[chid - 1].begin();
               it != rois_u_tight[chid - 1].end(); it++) {
        ...
        if (chid > nwire_u) {
          // form connectivity map
          for (auto it = rois_v_tight[chid - nwire_u - 1].begin();
               it != rois_v_tight[chid - nwire_u - 1].end(); it++) {
            SignalROI *prev_roi = *it;
            if (tight_roi->overlap(prev_roi)) {
              if (front_rois.find(prev_roi) == front_rois.end()) {
                SignalROISelection temp_rois;
...
void ROI_refinement::CleanUpROIs(int plane) {
  ...
  if (plane == 0) {
    LogDebug("CleanUpROIs :  {148, " << rois_u_loose[148].size());
    // int counter = 0;
...
void ROI_refinement::CheckROIs(int plane, ROI_formation &roi_form) {
  ...
  if (plane == 0) {
    ...
    LogDebug("CheckROIs :  {148, " << rois_u_loose[148].size());
...
void ROI_refinement::BreakROI(SignalROI *roi, float rms) {
  ...
  // double protection_enhance_factor = 10.;
  auto protected_zones =
      proteced_rois.equal_range({roi->get_chid(), roi->get_start_bin()});
  if (protected_zones.first!=protected_zones.second) {
    return;
  ...
  for (size_t i = 0; i != temp_signal.size(); i++) {
    ...
    // for (auto iter = protected_zones.first; iter != protected_zones.second;
    //      ++iter) {
    //   auto zone = iter->second;
    //   for (auto idx = zone.first - start_bin; idx < zone.second - start_bin;
    //        ++idx) {
    //     if (idx < 0 or idx >= (int)temp_signal.size())
    //       break;
    //     temp_signal.at(idx) *= protection_enhance_factor;
    //   }
    // }
  }
  ...
  LogDebug("{" << roi->get_chid() << ", " << roi->get_start_bin()  << "} : nfound: " << nfound);

  if (nfound == max_npeaks) {
    log->debug("ROI_refinement: local ch index {} (plane {}), found max peaks "
               "{} with threshold={}",
               roi->get_chid(), roi->get_plane(), nfound, th_percent);
  }

  // if multiple peaks found
  if (nfound > 1) {
    ...
    LogDebug("{" << roi->get_chid() << ", " << roi->get_start_bin()  << "} : npeaks_threshold: " << npeaks_threshold);
    ...
    if (npeaks_threshold > 1) {
      ...
      //  	  std::cout << valley_pos1[j] << " " << peak_pos1[j] << " " <<
      //  valley_pos1[j+1] << std::endl ;
      //  	}
      // }
      ...
      // Fill gap with contained tight ROI
      ...
      // Fill gaps with protected zones
      // for (auto iter = protected_zones.first; iter != protected_zones.second;
      //      ++iter) {
      //   int start =
      //       iter->second.first > start_bin ? iter->second.first - start_bin : 0;
      //   int end = iter->second.second < end_bin
      //                 ? iter->second.second - start_bin
      //                 : end_bin - 1;
      //   std::cout << "[yuhw] : " << start << ", " << end << std::endl;
      //   std::cout << "[yuhw] : " << iter->second.first << ", "
  ...
  for (int qx = 0; qx != 2; qx++) {
...

    {
      ...
      temp_signal.resize(temp1_signal.size(), 0);
      //      htemp->Reset();
      // std::cout << saved_b.size() << " " << bins.size() << " " <<
      // htemp->GetNbinsX() << std::endl;
      for (int j = 0; j < int(saved_b.size()) - 1; j++) {
        // if (irow==1240)
        //   std::cout << saved_b.size() << " " << j << " " << saved<<
        //   std::endl;

        // int flag = 0;
        int start_pos = saved_b.at(j);
        float start_content =
            temp1_signal.at(start_pos); // htemp1->GetBinContent(start_pos+1);
        int end_pos = saved_b.at(j + 1);
        float end_content =
            temp1_signal.at(end_pos); // htemp1->GetBinContent(end_pos+1);
...
void ROI_refinement::BreakROI1(SignalROI *roi) {
  ...
  LogDebug("{" << chid << ", " << roi->get_start_bin() << "} "
                       << roi->get_end_bin() << " " << bins.size() << " {"
                       << bins.front() << ", " << bins.back() << "} ");
...
void ROI_refinement::MultiPlaneProtection(const int plane,
            ...
            if (abs(pitch - index) < wire_resolution) {
              ...
              if (proteced_rois.find(key) == proteced_rois.end()) {
                ...
                if (sta - last_end <= tick_resolution) {
                  ...
                  std::cout << "split: {" << key.first << ", " << key.second
                            << "} " << std::endl;
                }
              }
              //  std::cout
...

  {
    ...
    for (auto ch : tmp) {
      std::cout << ch.first << " : {" << ch.second.first.first << ", "
                << ch.second.first.second << "} : {" << ch.second.second.first
                << ", " << ch.second.second.second << "}, " << std::endl;
    }
  }
}
