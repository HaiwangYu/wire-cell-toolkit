...
WIRECELL_FACTORY(PerChannelResponse, WireCell::SigProc::PerChannelResponse,
...
SigProc::PerChannelResponse::default_configuration() const
{
    Configuration cfg;
...
void SigProc::PerChannelResponse::configure(
    const WireCell::Configuration &cfg)
{
    ...
    for (auto jchresp : jchannels)
    {
        ...
        {
            THROW(
                ValueError() << errmsg{"zero length response in file " + m_filename});
        }
...
SigProc::PerChannelResponse::channel_response(int channel_ident) const
{
    const auto &it = m_cr.find(channel_ident);
    if (it == m_cr.end())
    {
        THROW(KeyError() << errmsg{
                  String::format("no response for channel %d", channel_ident)});
    }
    return it->second;
}
