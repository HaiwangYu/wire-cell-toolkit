...
WIRECELL_FACTORY(TbbFlow, WireCellTbb::TbbFlow, WireCell::IApplication,
...
void TbbFlow::configure(const Configuration &cfg)
{
    std::string type, name,
        desc = get<std::string>(cfg, "dfp", "TbbDataFlowGraph");
    ...
    for (auto thc : m_dfpgraph.connections())
    {
        ...
        l->debug("TbbFlow: Connect: {}:{} ( {} -> {} ) {}:{}", tail_tn.type,
                 tail_tn.name, conn.tail, conn.head, head_tn.type, head_tn.name);

        INode::pointer tail_node =
            WireCell::Factory::lookup<INode>(tail_tn.type, tail_tn.name);
        INode::pointer head_node =
            WireCell::Factory::lookup<INode>(head_tn.type, head_tn.name);
