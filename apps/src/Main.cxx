...
int Main::cmdline(int argc, char *argv[])
{
    ...
    desc.add_options()("help,h", "wire-cell [options] [arguments]")(
        ...
        "log sink, filename or 'stdout' or 'stderr', if added ':level' then set "
        "a log level for the sink")(
        "loglevel,L", po::value<vector<string>>(),
        "set lowest log level for a log in form 'name:level' or just 'level' for "
        "all (level one of critical,error,warn,info,debug,trace)")(
        ...
        //	("jsonpath,j", po::value< vector<string> >(),"specify a JSON
        //path=value")
...
void Main::initialize()
{
    ...
    for (auto c : m_cfgmgr.all())
    {
        ...
        if (c["type"].isNull())
        {
            ...
            THROW(ValueError() << errmsg{
                      "got configuration sequence element lacking a type"});
        }
    ...
    for (auto c : m_cfgmgr.all())
    {
        ...
        auto cfgobj =
            Factory::find_maybe<IConfigurable>(type, name);  // doesn't throw.
        if (!cfgobj)
        {
            continue;
        }
