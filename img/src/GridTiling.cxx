...
WIRECELL_FACTORY(GridTiling, WireCell::Img::GridTiling, WireCell::ITiling,
...
bool Img::GridTiling::operator()(const input_pointer &slice,
                                 output_pointer &out)
{
    out = nullptr;
    if (!slice)
    ...
    {
        SPDLOG_LOGGER_TRACE(
            l, "GridTiling: anode:{} face:{} slice:{} too few activities given",
            anodeid, faceid, slice->ident());
        return true;
    }
    ...
    for (const auto &chv : slice->activity())
    {
        for (const auto &wire : chv.first->wires())
        {
            ...
            if (wpid.face() != faceid)
            {
                // l->trace("anode:{} face:{} slice:{} chan:{} wip:{} q:{} skip {}",
                // anodeid, faceid, slice->ident(), chv.first->ident(), wire->index(),
                // chv.second, wpid);
                continue;
            }
            // l->trace("anode:{} face:{} slice:{} chan:{} wip:{} q:{} keep {}",
            // anodeid, faceid, slice->ident(), chv.first->ident(), wire->index(),
            // chv.second, wpid);
    ...
    for (size_t ind = 0; ind < measures.size(); ++ind)
    {
        ...
        if (blah.empty())
        {
            SPDLOG_LOGGER_TRACE(l,
                                "GridTiling: anode:{} face:{} slice:{} empty active "
                                "layer ind={} out of {}",
                                anodeid, faceid, slice->ident(), ind,
                                measures.size());
            continue;
        }
        ++nactive_layers;
    ...
    {
        SPDLOG_LOGGER_TRACE(l,
                            "GridTiling: anode:{} face:{} slice:{} missing active "
                            "layers {}  != {}, {} activities",
                            anodeid, faceid, slice->ident(), nactive_layers,
                            measures.size(), nactivities);
        return true;
    }
    ...
    {
        SimpleBlob *sb =
            new SimpleBlob(m_blobs_seen++, blob_value, 0.0, blob, slice, m_face);
