...
static double blob_weight(IBlob::pointer iblob, ISlice::pointer islice,
                        ...
                        ISlice::pointer islice)
{
    ...
    for (IBlob::pointer iblob : neighbors_oftype<IBlob::pointer>(grind, islice))
    {
        ...
        for (auto neigh : grind.neighbors(iblob))
        {
            if (neigh.code() == 'm')
            {
                IChannel::shared_vector imeas =
                    std::get<IChannel::shared_vector>(neigh.ptr);
    ...
    for (size_t ind = 0; ind < nblobs; ++ind)
    {
        ...
        IBlob::pointer nblob = std::make_shared<SimpleBlob>(
            oblob->ident(), value, unc, oblob->shape(), islice, oblob->face());
        grind.replace(oblob, nblob);
    }
}

bool Img::BlobSolving::operator()(const input_pointer &in,
                                  output_pointer &out)
{
    if (!in)
    {
