...
int main(const int argc, char *argv[])
{
    ...
    // This is the number of ionized electrons for a MIP assumed by MB noise
    // paper. note: with option "point" this is overridden below.
    ...
    for (int plane_id = 0; plane_id < 3; ++plane_id)
    {
        ...
        em("looked up " + pir_tns[plane_id]);
        {
            ...
            canvas->Update();
            // canvas->Print(Form("%s_%c_resp.png", out_basename.c_str(),
            // uvw[plane_id]));
            canvas->Print(pdfname.c_str(), "pdf");
        }
