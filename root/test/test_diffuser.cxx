...
{
    cerr << "L: [ " << smear->lpos(0) << " , " << smear->lpos(smear->lsize())
         << " ]" << endl;
    cerr << "L: [ " << smear->tpos(0) << " , " << smear->tpos(smear->tsize())
         << " ]" << endl;
...
void test_one()
{
    ...
    for (double mean = -100; mean <= 100; mean += 0.11)
    {
        ...
        // cerr << "mean=" << mean << " [" << bb.first << " --> " << bb.second <<
        // "]" << endl;
...
void test_plot_hist(MultiPdf &pdf)
{
    ...
    diffs.push_back(diff.diffuse(10 * binsize_l, 100 * binsize_t, 3 * sigma_l,
                                 3 * sigma_t, 10.0));
    diffs.push_back(diff.diffuse(100 * binsize_l, 10 * binsize_t, 3 * sigma_l,
                                 3 * sigma_t, 10.0));
    ...
    TH2F *h = new TH2F("smear", "Smear", (max_l - min_l) / binsize_l, min_l,
                       max_l, (max_t - min_t) / binsize_t, min_t, max_t);
    for (auto smear : diffs)
    {
        for (int tind = 0; tind < smear->tsize(); ++tind)
        {
            for (int lind = 0; lind < smear->lsize(); ++lind)
            {
                h->Fill(smear->lpos(lind, 0.5), smear->tpos(tind, 0.5),
                        smear->get(lind, tind));
            }
        }
    }
