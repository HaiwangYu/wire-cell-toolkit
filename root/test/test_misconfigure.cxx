...
std::vector<float> blip(Binning bins, double time = 1 * units::ms,
                        double sigma = 3 * units::us, double mag = 10000.0)
{
...
TH1F *plot_wave(MultiPdf &pdf, const std::vector<float> &wave, std::string name,
                std::string title)
{
    TH1F *hist =
...
TH2F *plot_frame(MultiPdf &pdf, IFrame::pointer frame, std::string name,
                 double mtick = -1, double mchan = -1);
TH2F *plot_frame(MultiPdf &pdf, IFrame::pointer frame, std::string name,
                 double mtick, double mchan)
{
    ...
    TH2F *hist = new TH2F(name.c_str(), name.c_str(), maxtick + 1, 0, maxtick + 1,
...
int main(int argc, char *argv[])
{
    ...
    {
        IConfigurable::pointer mccfg =
            Factory::lookup<IConfigurable>("Misconfigure");
    ...
    plot_wave(pdf, Waveform::real(resp_spec), "respect",
              "Electronics response spectrum - 200 bins");
    plot_wave(pdf, Waveform::real(resp_spec2), "respect2",
              "Electronics response spectrum - 400 bins");
    plot_wave(pdf, Waveform::real(resp_spec3), "respect3",
              "Electronics response spectrum - 50 bins");
