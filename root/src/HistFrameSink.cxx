...
bool Root::HistFrameSink::operator()(const IFrame::pointer &frame)
{
    ...
    typedef std::tuple<ITrace::vector, std::vector<int>, std::vector<int>>
        tct_tuple;
    ...
    for (ITrace::pointer trace : *traces)
    {
        ...
        if (wpident < 0)
        {
            cerr << "Channel " << ch << " has illegal wire plane ident: " << wpid
                 << endl;
        }
    }
    ...
    for (auto &thisplane : perplane)
    {
        ...
        TH2F *hist =
            new TH2F(Form("plane%d", wpident), Form("Plane %d", wpident), ntbins,
                     tmin / units::us, tmax / units::us, nchbins, chmin, chmax);
        ...
        for (auto &trace : traces)
        {
            ...
            {
                const double t =
                    t0 + (tick) * (tbin + ibin + 0.5);  // 0.5 to land in bin-center
        ...
        cerr << wpident << " ntraces:" << traces.size() << " "
             ...
             << " qtot:" << qtot / m_units << " "
             << " qunit:" << m_units << " "
             << " integ:" << hist->Integral() << " min:" << hist->GetMinimum()
             << " max:" << hist->GetMaximum() << " chan:" << nchbins << "[" << chmin
             << "," << chmax << "] "
             << " time:" << ntbins << "[" << tmin / units::us << ","
             << tmax / units::us << "]us\n";
