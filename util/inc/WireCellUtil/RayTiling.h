    ...
    namespace RayGrid
    {
        ...
        class Activity
        {
           public:
            ...
            Activity(layer_index_t layer, const range_t &span, int offset = 0,
                     double threshold = 0.0);

            // Create an activity starting at offset and with each
            // subsequent span elements set to value.
            Activity(layer_index_t layer, size_t span, double value, int offset = 0,
                     double threshold = 0.0);
        ...
        {
            os << "<strip L" << s.layer << " pind:[" << s.bounds.first << ","
               << s.bounds.second << "]>";
            return os;
        }
                                        ...
                                        const WireCell::RayGrid::Activity &a)
        {
            ...
            os << "<activity L" << a.layer() << " " << strips.size()
               << " strips over pind:[" << b << "," << e << "]>";
            return os;
        }
        ...
        {
            os << "<blob " << b.strips().size() << " strips, " << b.corners().size()
               << " corners>";
            return os;
        }
