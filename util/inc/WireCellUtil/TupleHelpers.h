...
/// If we are still before C++14, supply the fodder for doing the "indices
/// trick".
 ...
 */
   ...
   */
        ...
        any_vector_type as_any_impl(const tuple_type &t,
   ...
   */
        ...
        {
            return as_any_impl(
                t, std::make_index_sequence<std::tuple_size<tuple_type>::value>{});
        }

        // internal
        template <std::size_t... Indices>
        tuple_type from_any_impl(const any_vector_type &anyv,
                                 std::index_sequence<Indices...>)
        {
            return std::make_tuple(boost::any_cast<Types>(anyv[Indices])...);
        }

        /** Return a tuple made from the contents of an equivalent vector of
   * boost::any
   *
   ...
   */
        ...
        {
            return from_any_impl(
                anyv, std::make_index_sequence<std::tuple_size<tuple_type>::value>{});
        }
 ...
 */
    ...
    struct shared_queued<T<Types...>>
    {
        ...
        any_queue_type
        as_any_queue_convert(const std::deque<std::shared_ptr<const Element>> &in)
        {
        ...
        template <std::size_t... Indices>
        std::vector<any_queue_type>
        as_any_queue_impl(const shared_queued_tuple_type &toq,
                          std::index_sequence<Indices...>)
        {
   ...
   *
   *     typedef typename tuple_helper<IFDCS>::Wrapped<std::deque>::type
   * IFDCS_queues; IFDCS_queues qs; auto any_q = as_any_queue(qs); cerr <<
   * "First element from each queue:\n"; cerr << any_cast<int>(any_q[0][0]) <<
   * endl; cerr << any_cast<float>(any_q[1][0]) << endl; cerr <<
   * any_cast<double>(any_q[2][0]) << endl; cerr << any_cast<char>(any_q[3][0])
   * << endl; cerr << any_cast<std::string>(any_q[4][0]) << endl;
   */
        std::vector<any_queue_type>
        as_any_queue(const shared_queued_tuple_type &toq)
        {
            return as_any_queue_impl(
                toq, std::make_index_sequence<std::tuple_size<tuple_type>::value>{});
        }

        // internal, cast an any queue to a deque of shared_ptrs of Type.
        template <typename Type>
        std::deque<std::shared_ptr<const Type>>
        from_any_queue_convert(const any_queue_type &aq)
        {
        ...
        // internal, index a vector of any queues returning a typed
        // deque<shared_ptr<Type>>.
        template <std::size_t... Indices>
        shared_queued_tuple_type
        from_any_queue_impl(const std::vector<any_queue_type> &vaq,
                            std::index_sequence<Indices...>)
        {
   ...
   */
        shared_queued_tuple_type
        from_any_queue(const std::vector<any_queue_type> &vaq)
        {
            return from_any_queue_impl(
                vaq, std::make_index_sequence<std::tuple_size<tuple_type>::value>{});
        }
 ...
 */
    ...
    struct type_repeater
    {
        typedef decltype(std::tuple_cat(
            std::tuple<T>(), typename type_repeater<N - 1, T>::type())) type;
    };
    template <typename T>
