...
Activity::Activity(layer_index_t layer, size_t span, double value, int offset,
                   double threshold)
  : m_span(span, value)
  , m_layer(layer)
  , m_offset(offset)
...
{
    return Strip{m_layer,
                 std::make_pair(pitch_index(r.first), pitch_index(r.second))};
}
...
Activity Tiling::projection(const Blob &blob, const Activity &activity)
{
    ...
    {
        const double p =
            m_coords.pitch_location(c.first, c.second, activity.layer());
        pitches.push_back(p);
    }
    if (pitches.empty())
    {
    ...
    for (auto &blob : blobs)
    {
        ...
        for (const auto &corner : blob.corners())
        {
            ...
            for (int layer = 0; layer < nlayers; ++layer)
            {
                ...
                const double ploc =
                    coords.pitch_location(corner.first, corner.second, layer);
