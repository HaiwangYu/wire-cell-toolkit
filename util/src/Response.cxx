 ...
 */
WireCell::Response::Schema::FieldResponse
WireCell::Response::Schema::load(const char *filename)
{
    if (!filename)
    {
    ...
    for (auto plane : fr["planes"])
    {
        ...
        for (auto path : plr["paths"])
        {
            ...
            auto wcpath = PathResponse(current, par["pitchpos"].asDouble(),
                                       par["wirepos"].asDouble());
            paths.push_back(wcpath);
        }

        PlaneResponse wcplr(paths, plr["planeid"].asInt(),
                            plr["location"].asDouble(), plr["pitch"].asDouble());

        planes.push_back(wcplr);
    }

    auto adir = fr["axis"];
    auto axis = WireCell::Vector(adir[0].asDouble(), adir[1].asDouble(),
                                 adir[2].asDouble());
    auto ret = FieldResponse(planes, axis, fr["origin"].asDouble(),
                             fr["tstart"].asDouble(), fr["period"].asDouble(),
                             fr["speed"].asDouble());
    return ret;
}

void Response::Schema::dump(const char *filename,
                            const Response::Schema::FieldResponse &fr)
...
Response::wire_region_average(const Response::Schema::FieldResponse &fr)
{
    ...
    for (auto plane : fr.planes)
    {
        ...
        for (auto path : plane.paths)
        {
            ...
            else
            {
                ...
                {
                    fresp_map[eff_num].at(k) =
                        (fresp_map[eff_num].at(k) + path.current.at(k)) / 2.;
                }
            }
            if (fresp_map.find(-eff_num) == fresp_map.end())
            {
        ...
        for (size_t i = 0; i != pitch_pos.size(); i++)
        {
            ...
            if (i == 0)
            {
                pitch_pos_range_map[eff_num] = std::make_pair(
                    min, (pitch_pos.at(i) + pitch_pos.at(i + 1)) / 2. * 0.01 * pitch);
            }
            else if (i == pitch_pos.size() - 1)
            {
                pitch_pos_range_map[eff_num] = std::make_pair(
                    (pitch_pos.at(i) + pitch_pos.at(i - 1)) / 2. * 0.01 * pitch, max);
            }
            else
            {
                pitch_pos_range_map[eff_num] = std::make_pair(
                    (pitch_pos.at(i) + pitch_pos.at(i - 1)) / 2. * 0.01 * pitch,
                    (pitch_pos.at(i) + pitch_pos.at(i + 1)) / 2. * 0.01 * pitch);
            }
        }
        ...
        for (size_t i = 0; i != pitch_pos.size(); i++)
        {
            ...
            {
                wire_regions.insert(
                    round((pitch_pos.at(i) * 0.01 * pitch - 0.001 * pitch) / pitch));
            }
            else
            {
                wire_regions.insert(
        ...
        for (auto it = wire_regions.begin(); it != wire_regions.end(); it++)
        {
            ...
            for (auto it1 = fresp_map.begin(); it1 != fresp_map.end(); it1++)
            {
                ...
                if (high_limit > low_limit)
                {
                    ...
                    {
                        avgs[wire_no].at(k) +=
                            response.at(k) * (high_limit - low_limit) / pitch;
                    }
                }
            }
        }
...
Response::Schema::FieldResponse
Response::average_1D(const Response::Schema::FieldResponse &fr)
{
    using namespace WireCell::Waveform;
    using namespace WireCell::Response::Schema;
...
Array::array_xxf Response::as_array(const Schema::PlaneResponse &pr,
                                    int set_nrows, int set_ncols)
{
    int nrows = pr.paths.size();
    int ncols = pr.paths[0].current.size();
    Array::array_xxf ret =
        Array::array_xxf::Zero(set_nrows, set_ncols);  // warning, uninitialized
    ...
    for (int irow = 0; irow < nrows; ++irow)
    {
        if (irow < set_nrows)
        {
            ...
            for (int icol = 0; icol < ncols; ++icol)
            {
                if (icol < set_ncols)
                    ret(irow, icol) =
                        path.current[icol];  // maybe there is a fast way to do this copy?
            }
        }
    }
    return ret;
...
Response::Generator::generate(const WireCell::Waveform::Domain &domain,
                              int nsamples)
{
...
WireCell::Waveform::realseq_t
Response::Generator::generate(const WireCell::Binning &tbins)
{
...
{
    if (time <= 0 || time >= 10 * units::microsecond)
    {  // range of validity
        return 0.0;
    }
    ...
    return 4.31054 * exp(-2.94809 * reltime) * gain -
           ...
           2.6202 * exp(-2.82833 * reltime) * cos(1.19361 * reltime) *
               cos(2.38722 * reltime) * gain +
           0.464924 * exp(-2.40318 * reltime) * cos(2.5928 * reltime) * gain +
           0.464924 * exp(-2.40318 * reltime) * cos(2.5928 * reltime) *
               cos(5.18561 * reltime) * gain +
           0.762456 * exp(-2.82833 * reltime) * sin(1.19361 * reltime) * gain -
           0.762456 * exp(-2.82833 * reltime) * cos(2.38722 * reltime) *
               sin(1.19361 * reltime) * gain +
           0.762456 * exp(-2.82833 * reltime) * cos(1.19361 * reltime) *
               sin(2.38722 * reltime) * gain -
           2.620200 * exp(-2.82833 * reltime) * sin(1.19361 * reltime) *
               sin(2.38722 * reltime) * gain -
           0.327684 * exp(-2.40318 * reltime) * sin(2.5928 * reltime) * gain +
           +0.327684 * exp(-2.40318 * reltime) * cos(5.18561 * reltime) *
               sin(2.5928 * reltime) * gain -
           0.327684 * exp(-2.40318 * reltime) * cos(2.5928 * reltime) *
               sin(5.18561 * reltime) * gain +
           0.464924 * exp(-2.40318 * reltime) * sin(2.5928 * reltime) *
               sin(5.18561 * reltime) * gain;
}
...
double Response::SimpleRC::operator()(double time) const
{
    ...
    {
        ret += -_tick / _width *
               exp(-(time - _offset) / _width);  // _tick here is to make this RC
                                                 // response integrated in each bin
    }
    if (time < _offset + _tick)
    {                // just the first bin
...
Response::SysResp::SysResp(double tick, double magnitude, double smear,
                           double offset)
  : _tick(tick)
  , _mag(magnitude)
  , _smear(smear)
...
double Response::SysResp::operator()(double time) const
{
    ...
    if (_smear > 0)
    {
        ret = _tick * exp(-0.5 * pow((time - _offset) / _smear, 2)) / _smear *
              0.3989422804;
    }
