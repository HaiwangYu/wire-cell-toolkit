  ...
  , log(Log::logger("sim"))
{
    ...
    for (int i = 0; i != m_num_group; i++) {
        ...
        // Eigen::SparseMatrix<float> *mat = new
        // Eigen::SparseMatrix<float>(nsamples,nwires);
    ...
    //  std::cout << impact_range.first << " " << impact_range.second << " " <<
    ...
    // std::cout << start_ch << " " << end_ch << " " << npad_wire << " " <<
    // start_tick << " " << end_tick << " " << m_start_ch << " " << m_end_ch <<
    // std::endl;
    ...
    for (int i = 0; i != num_double; i++) {
        ...
        //   it(*m_vec_spmatrix.at(i),k); it; ++it){
        ...
        //   for (Eigen::SparseMatrix<float>::InnerIterator
        //   it(*m_vec_spmatrix.at(ii),k); it; ++it){
        // 	c_data(it.col()+npad_wire-start_ch,it.row()-m_start_tick) = it.value();
        //   }
    ...
    // central region ...
    {
        ...
        Array::array_xxc data_f_w;
        {
            ...
            //      std::cout << i << " " << m_vec_vec_charge.at(i).size() <<
            //      std::endl;
            m_vec_vec_charge.at(i).clear();
            m_vec_vec_charge.at(i).shrink_to_fit();
            // for (int k=0; k<m_vec_spmatrix.at(i)->outerSize(); ++k)
            // 	for (Eigen::SparseMatrix<float>::InnerIterator
            // it(*m_vec_spmatrix.at(i),k); it; ++it){
            // 	  data_t_w(it.col()+npad_wire-start_ch,it.row()-m_start_tick) =
            // it.value();
            // 	}
            // delete m_vec_spmatrix.at(i);
            // //      m_vec_spmatrix.at(i).setZero();
...

        {
            ...
            {
                Waveform::compseq_t rs1 = m_vec_map_resp.at(i)[0]->spectrum();
                // Array::array_xxc temp_resp_f_w =
                // Array::array_xxc::Zero(2*m_num_pad_wire+1,nsamples); for (int icol =
                // 0; icol != nsamples; icol++){
                //   temp_resp_f_w(0,icol) = rs1[icol];
                // }
                // Array::array_xxf temp_resp_t_w =
                // Array::idft_cr(temp_resp_f_w,0).block(0,0,2*m_num_pad_wire+1,m_end_tick-m_start_tick);
    ...
    // std::cout << real_m_decon_data(40,5182) << " " << img_m_decon_data(40,5182)
    // << std::endl;
    //    std::cout << real_m_decon_data(40,5182-m_start_tick) << " " <<
    //    img_m_decon_data(40,5182-m_start_tick) << std::endl;

    //}else{
    // Array::array_xxc temp_m_decon_data = Array::idft_cc(acc_data_f_w,0);
    //   Array::array_xxf real_m_decon_data = temp_m_decon_data.real();
    //   Array::array_xxf img_m_decon_data =
    //   temp_m_decon_data.imag().rowwise().reverse(); m_decon_data =
    //   real_m_decon_data + img_m_decon_data;
    // }
    ...
    //   {
    // 	Array::array_xxf data_t_w =
    // Array::array_xxf::Zero(nwires+2*npad_wire,nsamples);
    // 	// fill charge array in time-wire domain // slightly larger
    // 	for (size_t j=0;j!=m_vec_vec_charge.at(i).size();j++){
    ...
    //   {
    // 	Array::array_xxc resp_f_w =
    // Array::array_xxc::Zero(nwires+2*npad_wire,nsamples);
    // 	{
    // 	  Waveform::compseq_t rs1 = m_vec_map_resp.at(i)[0]->spectrum();
    // 	  for (int icol = 0; icol != nsamples; icol++){
    // 	    resp_f_w(0,icol) = rs1[icol];
    ...
    // 	}
    // 	// Do FFT on wire for response // slight larger
    // 	resp_f_w = Array::dft_cc(resp_f_w,1); // Now becomes the f and f in both
    // time and wire domain ...
    // 	// multiply them together
    // 	data_f_w = data_f_w * resp_f_w;
    //   }
...
Waveform::realseq_t Gen::ImpactTransform::waveform(int iwire) const
{
    ...
    if (iwire < m_start_ch || iwire >= m_end_ch) {
        ...
        if (m_pir->closest(0)->long_aux_waveform().size() > 0) {
            ...
            //   std::cout << nlength << " " << nsamples +
            //   m_pir->closest(0)->long_aux_waveform_pad() << std::endl;
