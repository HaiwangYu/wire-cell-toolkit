...
WIRECELL_FACTORY(Ductor, WireCell::Gen::Ductor, WireCell::IDuctor,
...
void Gen::Ductor::configure(const WireCell::Configuration &cfg)
{
    ...
    if (jpirs.isNull() or jpirs.empty())
    {
        ...
        THROW(ValueError() << errmsg{"Gen::Ductor: must configure with some plane "
                                     "impact response components"});
    }
    m_pirs.clear();
    for (auto jpir : jpirs)
    {
    ...
    l->debug(
        "AnodePlane: {}, mode: {}, fluctuate: {}, time start: {} ms, "
        "readout time: {} ms, frame start: {}",
        m_anode_tn, m_mode, (m_fluctuate ? "on" : "off"),
        m_start_time / units::ms, m_readout_time / units::ms, m_frame_count);
}
...
void Gen::Ductor::process(output_queue &frames)
{
    ...
    frame->tag_traces(string("ductor") + std::to_string(m_anode->ident()),
                      indices);
    frame->tag_frame("ductor");
    frames.push_back(frame);
    l->debug("made frame: {} with {} traces @ {}ms", m_frame_count, traces.size(),
             m_start_time / units::ms);
