...
Waveform::realseq_t Gen::ImpactZipper::waveform(int iwire) const
{
    ...
    for (int imp = min_impact; imp <= max_impact; ++imp)
    {
        ...
        // std::cerr << "IZ: " << " imp=" << imp << " imp_pos=" << imp_pos << "
        ...
        ++nfound;
        // std::cerr << "ImpactZipper: found:"<<nfound<<" for absolute impact number
        // " << imp
        //           << " csize=" << csize << " rsize=" << rsize << " rebin=" <<
        //           rebinfactor
        //           << std::endl;

        Waveform::increase(total_spectrum, conv_spectrum);
    }
    // std::cerr << "ImpactZipper: found " << nfound << " in abs impact: ["  <<
