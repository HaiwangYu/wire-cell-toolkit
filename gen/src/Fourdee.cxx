...
WIRECELL_FACTORY(FourDee, WireCell::Gen::Fourdee, WireCell::IApplication,
...
void Gen::Fourdee::configure(const Configuration &thecfg)
{
    ...
    if (tn.empty())
    {  // digitizer is optional, voltage saved w.o. it.
        m_digitizer = nullptr;
        cerr << "\tDigitizer: none\n";
    }
    else
...
void dump(const IFrame::pointer frame)
{
    ...
    for (auto tag : frame->frame_tags())
    {
        ...
        cerr << "Fourdee: frame tag: " << tag << " with " << tlist.size()
             << " traces\n";
    }
    for (auto tag : frame->trace_tags())
    {
        const auto &tlist = frame->tagged_traces(tag);
        cerr << "Fourdee: trace tag: " << tag << " with " << tlist.size()
             << " traces\n";
    }
    ...
    cerr << "frame: #" << frame->ident() << " @" << frame->time() / units::ms
...
void dump(DEPOS &depos)
{
    ...
    std::cerr << "Gen::FourDee: drifted " << ndepos << ", extent:\n"
              << "\tt in [ " << (*tmm.first) / units::us << ","
              << (*tmm.second) / units::us << "]us,\n"
              << "\tx in [" << (*xmm.first) / units::mm << ","
              << (*xmm.second) / units::mm << "]mm,\n"
              << "\ty in [" << (*ymm.first) / units::mm << ","
              << (*ymm.second) / units::mm << "]mm,\n"
              << "\tz in [" << (*zmm.first) / units::mm << ","
              << (*zmm.second) / units::mm << "]mm,\n"
              << "\tQtot=" << qtot / units::eplus
              << ", <Qtot>=" << qtot / ndepos / units::eplus << " "
              << " Qorig=" << qorig / units::eplus
              << ", <Qorig>=" << qorig / ndepos / units::eplus << " electrons"
              << std::endl;
}
   ...
   public:
    ...
    virtual bool operator()()
    {
        ...
        const IFrame::pointer iframe =
            boost::any_cast<const IFrame::pointer>(iq.front());
        iq.pop();
...
void Gen::Fourdee::execute_new()
{
    ...
    if (!m_ductor and
        (m_digitizer or m_dissonance or m_digitizer or m_filter or m_output))
    {
        std::cerr
    ...
    if (m_drifter)
    {  // depo in, depo out
        cerr << "Pipeline adding #" << pipeline.size() << ": " << type(*m_drifter)
             << endl;
    ...
    if (m_depofilter)
    {  // depo in, depo out
        cerr << "Pipeline adding #" << pipeline.size() << ": "
             << type(*m_depofilter) << endl;
        auto proc = new FunctionNodeProc(m_depofilter);
        last_proc = join(pipeline, last_proc, proc);
    }

    if (m_ductor)
    {  // depo in, zero or more frames out
        cerr << "Pipeline adding #" << pipeline.size() << ": " << type(*m_ductor)
             << endl;
    ...
    if (m_digitizer)
    {  // frame in, frame out
        cerr << "Pipeline adding #" << pipeline.size() << ": " << type(*m_digitizer)
             << endl;
    ...
    if (m_filter)
    {  // frame in, frame out
        cerr << "Pipeline adding #" << pipeline.size() << ": " << type(*m_filter)
             << endl;
    ...
    if (m_output)
    {  // frame in, full stop.
        cerr << "Pipeline adding #" << pipeline.size() << ": " << type(*m_output)
             << endl;
    ...
    while (true)
    {
        ...
        for (size_t ind = pipelen - 1; ind > 0; --ind)
        {  // source has no input
...
{
    if (!m_depos)
        cerr << "Fourdee: no depos" << endl;
    if (!m_drifter)
        cerr << "Fourdee: no drifter" << endl;
    if (!m_ductor)
        cerr << "Fourdee: no ductor" << endl;
    if (!m_dissonance)
        cerr << "Fourdee: no dissonance" << endl;
    if (!m_digitizer)
        cerr << "Fourdee: no digitizer" << endl;
    if (!m_filter)
        cerr << "Fourdee: no filter" << endl;
    if (!m_output)
        cerr << "Fourdee: no sink" << endl;
            ...
            for (IFrameFilter::input_pointer voltframe : frames)
            {
                ...
                if (m_dissonance)
                {
                    ...
                    if (noise)
                    {
                        ...
                        dump(noise);
                        voltframe =
                            Gen::sum(IFrame::vector{voltframe, noise}, voltframe->ident());
                        em("got noise");
                    }
                    else
                    {
                ...
                cerr << "Gen::Fourdee: frame with " << adcframe->traces()->size()
                     << " traces\n";
            }
        }
    }
