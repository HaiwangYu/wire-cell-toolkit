...
WIRECELL_FACTORY(Reframer, WireCell::Gen::Reframer, WireCell::IFrameFilter,
...
bool Gen::Reframer::operator()(const input_pointer &inframe,
                               output_pointer &outframe)
{
    ...
    outframe = make_shared<SimpleFrame>(
        inframe->ident(), inframe->time() + m_toffset + m_tbin * inframe->tick(),
        out_traces, inframe->tick());
    log->debug("Gen::Reframer: frame {} {} traces, {} ticks", inframe->ident(),
               out_traces.size(), m_nticks);
    return true;
}
