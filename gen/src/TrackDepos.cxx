...
WIRECELL_FACTORY(TrackDepos, WireCell::Gen::TrackDepos, WireCell::IDepoSource,
...
{
    std::stringstream ss;
    ss << "q=" << d->charge() / units::eplus
       << "eles, t=" << d->time() / units::us << "us, r=" << d->pos() / units::mm
       << "mm";
    return ss.str();
}

void Gen::TrackDepos::add_track(double time, const WireCell::Ray &ray,
                                double charge)
{
    l->debug("add_track({} us, ({} -> {})cm, {})", time / units::us,
             ray.first / units::cm, ray.second / units::cm, charge);
