...
// bool Gen::GausDiffTimeCompare::operator()(const
// std::shared_ptr<Gen::GaussianDiffusion>& lhs, const
...
bool Gen::BinnedDiffusion_transform::add(IDepo::pointer depo, double sigma_time, double sigma_pitch)
{
    ...
    // int bin_end = std::min(ibins.bin(center_pitch + sigma_pitch*m_nsigma)+1,
    // ibins.nbins()); debug
...
// void Gen::BinnedDiffusion_transform::add(std::shared_ptr<GaussianDiffusion>
// gd, int bin)
// {
...
//              <<", for bin " << bin << " t=[" << mm.first/units::us << "," <<
//              mm.second/units::us << "]us\n";
//     }
//     m_diffs.insert(gd);
//     //m_diffs.push_back(gd);
// }

// void Gen::BinnedDiffusion_transform::erase(int begin_impact_number, int
// end_impact_number)
// {
                                                       ...
                                                       std::vector<int> &vec_impact)
{
    ...
    for (int wireind = 0; wireind != rb.nbins(); wireind++) {
        ...
        for (int imp_no = imps_range.first; imp_no != imps_range.second; imp_no++) {
            ...
            //  std::cout << imp_no << " " << wireind << " " << wire_imp_no << " " <<
            //  ib.center(imp_no) << " " << rb.center(wireind) << " " <<
            //  ib.center(imp_no) - rb.center(wireind) << std::endl;
            // std::cout << imp_no << " " << map_imp_ch[imp_no] << " " <<
            // map_imp_redimp[imp_no] << std::endl;
        }
    }
    ...
    std::vector<std::vector<std::tuple<int, int, double>>> &vec_vec_charge, std::vector<int> &vec_impact)
{
    ...
    //  int min_redimp =  m_pimpos.wire_impacts(2).first -
    //  m_pimpos.wire_impact(2); int max_redimp =  m_pimpos.wire_impacts(2).second
    //  - 1 - m_pimpos.wire_impact(2);
    int min_imp = 0;
    int max_imp = ib.nbins();

    // std::cout << min_redimp << " " << max_redimp << " " << max_imp <<
    // std::endl;
    ...
    for (auto diff : m_diffs) {
        ...
        for (int pbin = 0; pbin != np; pbin++) {
            ...
            for (int tbin = 0; tbin != nt; tbin++) {
                ...
                //   std::cout << pbin+poffset_bin << " " << pbin << " " << tbin << " "
                //   << charge << " " << std::endl;
                // }
...
// Gen::ImpactData::pointer Gen::BinnedDiffusion_transform::impact_data(int bin)
// const
// {
//     const auto ib = m_pimpos.impact_binning();
//     if (! ib.inbounds(bin)) {
