...
WIRECELL_FACTORY(Retagger, WireCell::Gen::Retagger, WireCell::IFrameFilter,
...
bool Gen::Retagger::operator()(const input_pointer &inframe,
                               output_pointer &outframe)
{
    ...
    auto sfout = new SimpleFrame(inframe->ident(), inframe->time(),
                                 *inframe->traces(), inframe->tick());
    ...
    //
    // merge
    typedef std::tuple<tagrules::tag_t, IFrame::trace_list_t,
                       IFrame::trace_summary_t>
        tag_trace_list_summary_t;
    ...
    for (auto inttag : inframe->trace_tags())
    {
        ...
        // std::cerr << "Retagger: " << inttag << " -> " << touttags.size() << "
        // outtags\n";
        if (touttags.empty())
        {
            continue;
    ...
    for (auto it : stashmap)
    {
        ...
        {  // one more time
            ...
            if (summary.empty())
            {
                ...
                {
                    summary.push_back(0);  // zero pad to cover missing summary and keep
                                           // alignment with traces.
                }
            }
            else
            {
        ...
        std::cerr << "Retagger: tagging trace set: " << otag << " with "
                  << otraces.size() << " traces, " << osummary.size()
                  << " summary\n";
