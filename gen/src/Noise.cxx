...
Waveform::realseq_t
Gen::Noise::generate_waveform(const std::vector<float> &spec,
                              IRandom::pointer rng, double replace)
{
    ...
    for (int i = shift; i < int(spec.size()); i++)
    {
        const double amplitude =
            spec.at(i - shift) * sqrt(2. / 3.1415926);  // / units::mV;
        noise_freq.at(i - shift).real(random_real_part.at(i) * amplitude);
        noise_freq.at(i - shift).imag(
            random_imag_part.at(i) * amplitude);  //= complex_t(real_part,imag_part);
    }
    for (int i = 0; i < shift; i++)
    {
        const double amplitude =
            spec.at(i + int(spec.size()) - shift) * sqrt(2. / 3.1415926);
        noise_freq.at(i + int(spec.size()) - shift)
            .real(random_real_part.at(i) * amplitude);
        noise_freq.at(i + int(spec.size()) - shift)
            .imag(random_imag_part.at(i) * amplitude);
    }
