...
// std::pair<int,int> Gen::GausDesc::subsample_range(int nsamples, double xmin,
// double xmax, double nsigma) const
// {
    ...
    if (fluctuate)
    {
        ...
        for (size_t ip = 0; ip < npss; ++ip)
        {
            for (size_t it = 0; it < ntss; ++it)
            {
                ...
                // distribution but n_i, n_j has covariance -n_tot * p_i * p_j normalize
                // later to approximate this multinomial distribution (how precise?) how
                // precise? better than poisson and 10000 total charge corresponds to a
                // <1% level error.
                double number = 0.0;
