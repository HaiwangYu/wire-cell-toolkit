...
WIRECELL_FACTORY(BlipSource, WireCell::Gen::BlipSource, WireCell::IDepoSource,
...
WireCell::Configuration Gen::BlipSource::default_configuration() const
{
    ...
    ene["value"] = 20000.0;
    cfg["charge"] = ene;
    // also, in Jsonnet can support PDFs giving probability for certain number of
    // electrons { type="pdf",
...
struct Pdf : public Gen::BlipSource::ScalarMaker
{
    ...
    double total;
    Pdf(IRandom::pointer rng, const std::vector<double> &pdf,
    ...
    double operator()()
    {
        ...
        for (size_t ind = 1; ind < cumulative.size();
             ++ind)
        {  // start at 2nd element
            if (cumulative[ind] >= cp)
            {
                double rel = (cp - cumulative[ind - 1]) /
...
struct UniformBox : public Gen::BlipSource::PointMaker
{
    ...
    Point operator()()
    {
        auto p = Point(rng->uniform(extent.first.x(), extent.second.x()),
...
void Gen::BlipSource::configure(const WireCell::Configuration &cfg)
{
    ...
    {
        m_ene = new Pdf(m_rng, get<std::vector<double>>(ene, "pdf"),
                        get<std::vector<double>>(ene, "edges"));
    }
    else
    {
...
bool Gen::BlipSource::operator()(IDepo::pointer &depo)
{
    ...
    if (m_time > m_stop)
    {
        std::cerr << "BlipSource: reached stop time: " << m_time / units::ms
                  << " > " << m_stop / units::ms << std::endl;
        depo = nullptr;
        m_eos = true;
        return true;
    }
    ++m_blip_count;
    depo = std::make_shared<SimpleDepo>(m_time, (*m_pos)(), (*m_ene)(), nullptr,
                                        0, 0, m_blip_count);
    return true;
}
