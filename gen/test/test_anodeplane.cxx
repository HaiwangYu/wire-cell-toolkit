    ...
    for (std::string anode_tn : anode_tns)
    {
        ...
        for (auto face : iap->faces())
        {
            ...
            for (auto plane : face->planes())
            {
                ...
                for (int axis : {0, 1, 2})
                {
                    cerr << "\taxis " << axis << ": " << pimpos->axis(axis) / units::mm
                         << "mm\n";
                }
            ...
            {
                cerr << "ERROR, field response and wire location data do not match: "
                        "diff = "
                     << diff / units::mm << "mm\n";
                cerr << "front: " << originx.front() / units::mm
                     << "mm, back=" << originx.back() / units::mm << "mm out of "
                     << originx.size() << endl;
