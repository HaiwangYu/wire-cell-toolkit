...
void test_depoplanex()
{
    ...
    for (auto t : times)
    {
        ...
        for (auto x : xes)
        {
            ...
            std::cerr << "\tx=" << x / units::cm
                      << "cm, t=" << newdepo->time() / tunit << "us\n";
        }
        std::cerr << "^ t=" << t / tunit
                  << "us, fot=" << dpx.freezeout_time() / tunit << "us\n";
    }
    dpx.freezeout();
    double fot = dpx.freezeout_time();
    std::cerr << dpx.frozen_queue().size() << " frozen with fot=" << fot / tunit
              << "us, " << dpx.working_queue().size() << " working\n";

    auto removed = dpx.pop(fot / 2.0);
    std::cerr << "Popped " << removed.size() << " at " << fot / 2.0 / tunit
              << std::endl;
    for (auto depo : removed)
    {
        std::cerr << "\tx=" << depo->pos().x() / units::cm
                  << "cm, t=" << depo->time() / tunit << "us";
        std::cerr << " <--- ";
        std::cerr << "x=" << depo->prior()->pos().x() / units::cm
                  << "cm, t=" << depo->prior()->time() / tunit << "us\n";
    }
