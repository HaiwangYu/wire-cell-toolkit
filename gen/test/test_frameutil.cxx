...
int main()
{
    ...
    auto f1 = make_shared<SimpleFrame>(signal_frame_ident, signal_start_time,
                                       signal_traces, tick);
    ...
    ITrace::vector noise_traces{make_shared<SimpleTrace>(0, 0, noise),
                                make_shared<SimpleTrace>(1, 0, noise),
                                make_shared<SimpleTrace>(2, 0, noise),
                                make_shared<SimpleTrace>(3, 0, noise)};
    auto f2 = make_shared<SimpleFrame>(noise_frame_ident, noise_start_time,
                                       noise_traces, tick);
    ...
    cerr << "Frame #" << f3->ident() << " time=" << f3->time() / units::ms
         << "ms\n";
    for (auto trace : *traces)
    {
        const auto &charge = trace->charge();
        cerr << "ch=" << trace->channel() << " tbin=" << trace->tbin()
             << " nsamples=" << charge.size() << endl;
