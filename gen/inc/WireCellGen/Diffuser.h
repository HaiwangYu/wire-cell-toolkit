 ...
 */
   ...
   * \param nsigma defines the number of sigma at which to truncate the
   * Gaussian.
   */
        Diffuser(const Ray &pitch = Ray(Point(0.0, 0.0, 0.0),
                                        Point(0.0, 0.0, 5 * units::millimeter)),
   ...
   * diffusion directly.
   */
        IDiffusion::pointer diffuse(double mean_l, double mean_t, double sigma_l,
                                    double sigma_t, double weight = 1.0,
                                    IDepo::pointer depo = nullptr);

        /* Internal function, return the +/- nsigma*sigma bounds
   * around mean enlarged to fall on bin edges.
   */
        bounds_type bounds(double mean, double sigma, double binsize,
                           double origin = 0.0);

        /* Internal function, return a 1D distribution of mean/sigma
   * binned with zeroth bin at origin and given binsize.
   */
        std::vector<double> oned(double mean, double sigma, double binsize,
