...
Configuration Hio::HDF5FrameSource::default_configuration() const
{
    ...
    cfg["filelist"] =
        Json::arrayValue;  // list of input files, empties are skipped
    cfg["policy"] = "";    // set to "stream" to avoid sending EOS after each file's
                           // worth of depos.
...
bool Hio::HDF5FrameSource::operator()(IFrame::pointer &out)
{
    ...
    while (!m_filenames.empty())
    {
        ...
        for (int sequence = 0; sequence < sequence_max; ++sequence)
        {
            ...
            eigen_to_traces(d.transpose(), *itraces, trace_index,
                            m_anode->channels().front(), nticks, false);

            SimpleFrame *sframe = new SimpleFrame(
                sequence, start_time, ITrace::shared_vector(itraces), tick);
