...
int FrameTools::frmtcmp(IFrame::pointer frame, double time)
{
    ...
    if (tmax <= time)
    {  // high edge may be exactly equal to target time and frame
        // does not span.
        return -1;
    }
    if (tmin >= time)
    {  // low edge may be exactly equal to target time and frame
        // does not span.
        return +1;
    }
    return 0;
}
std::pair<IFrame::pointer, IFrame::pointer>
FrameTools::split(IFrame::pointer frame, double time)
{
    ...
    IFrame::pointer mframe =
        std::make_shared<SimpleFrame>(ident, tref, mtraces, tick);
    IFrame::pointer pframe =
        std::make_shared<SimpleFrame>(ident, tref, ptraces, tick);
...
ITrace::vector FrameTools::tagged_traces(IFrame::pointer frame,
                                         IFrame::tag_t tag)
{
    if (tag == "")
    {
...
void FrameTools::fill(Array::array_xxf &array, const ITrace::vector &traces,
                      channel_list::iterator chit, channel_list::iterator chend,
                      int tbin)
{
    ...
    const int nrows =
        std::min((int) array.rows(), (int) std::distance(chit, chend));
